...
def main():
    ...
    print("="*80)
    cmd = [sys.executable, str(BASE_DIR / "tools" / "create_missing_tests.py")] + force_arg + dry_run_arg
    ...
    print("="*80)
    cmd = [sys.executable, str(BASE_DIR / "tools" / "update_test_files.py")] + force_arg + dry_run_arg
    rc, stdout, stderr = run_command(cmd)
    print(stdout)
    if stderr:
        print(f"Errors:\n{stderr}")
    ...
    if integration:
        ...
        print("="*80)
        cmd = [sys.executable, str(BASE_DIR / "tools" / "create_integration_tests.py")] + dry_run_arg
        rc, stdout, stderr = run_command(cmd)
        print(stdout)
        if stderr:
            print(f"Errors:\n{stderr}")
