...
def check_environment():
    ...
    if not (BASE_DIR / "sflib.py").exists():
        print("WARNING: sflib.py not found in base directory. Are you running from the correct location?")
    else:
...
def main():
    ...
    # Check if we should force updates
    force = '--force' in sys.argv
    force_arg = ['--force'] if force else []
    
    # 1. Create missing test files
    print("\n" + "="*80)
    print("STEP 1: Creating test files for modules without tests...")
    print("="*80)
    rc, stdout, stderr = run_command([sys.executable, str(BASE_DIR / "tools" / "create_missing_tests.py")] + force_arg)
    print(stdout)
    if stderr:
        print(f"Errors:\n{stderr}")
    ...
    print("="*80)
    rc, stdout, stderr = run_command([sys.executable, str(BASE_DIR / "tools" / "update_test_files.py")] + force_arg)
    print(stdout)
    if stderr:
        print(f"Errors:\n{stderr}")
    
    print("\nTest files upgrade complete!")
    print("\nYou can now run the tests with: pytest -xvs test/unit/modules/")
