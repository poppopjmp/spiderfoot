...
TEST_CLASS_RE = re.compile(r'class TestModule([a-zA-Z0-9_]*)\s*\(\s*SpiderFootModuleTestCase\s*\):')
MODULE_INSTANTIATION_RE = re.compile(r'(\s+)([a-zA-Z_]+)\s*=\s*([a-zA-Z0-9_]+)\(\)')
...
def import_module_class(module_path):
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location(Path(module_path_str).stem, module_path_str)
...
def update_test_file(file_path, dry_run=False, force=False):
    ...
    try:
        ...
        if "test_module_base.py" in file_path_str or "test_module_template.py" in file_path_str:
        ...
        test_class_match = re.search(r'class\s+(\w+)\s*\(\s*SpiderFootModuleTestCase\s*\):', content)
        ...
        if not force and 'self.module_class = self.create_module_wrapper(' in content:
            print(f"File {file_path_str} already uses wrapper pattern, skipping...")
            return False
        ...
        setup_content = SETUP_TEMPLATE.format(
            ...
            module_description=module_attrs.get('descr', "Module description unavailable"),
        ...
        basic_tests_exist = all(method in content for method in [
            'test_opts', 
            'test_setup', 
...
if __name__ == '__main__':
    ...
    print(f"{'DRY RUN: ' if dry_run else ''}Updating SpiderFoot test files to use the wrapper pattern...")
