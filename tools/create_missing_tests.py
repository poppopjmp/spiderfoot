...
def clean_module_name(name):
    ...
    if first_letter in special_prefixes:
        ...
        camel_case = first_letter + parts[0].capitalize() + ''.join(part.capitalize() for part in parts[1:])
    else:
...
def get_missing_test_files():
    ...
    module_files = [f for f in os.listdir(MODULES_DIR) if f.startswith('sfp_') and f.endswith('.py')]
    
    # Get all test files
    test_files = [f for f in os.listdir(TEST_DIR) if f.startswith('test_sfp_') and f.endswith('.py')]
...
if __name__ == '__main__':
    ...
    print(f"{'DRY RUN: ' if dry_run else ''}Creating missing test files for SpiderFoot modules...")
