...
import { Briefcase, Plus, Trash2, X, Target, Copy, CheckCircle2, FolderOpen, Clock, Edit2, Radar, Link2, Unlink, Brain, FileText, Sparkles, Edit3, Save, Loader2, AlertTriangle } from 'lucide-react';
import { useState, useRef, useEffect, useCallback } from 'react';
import { StatusBadge, Toast, type ToastType } from '../components/ui';
  ...
  return (
    <div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-dark-800 border border-dark-700 rounded-2xl p-6 max-w-md w-full shadow-2xl animate-fade-in-up">
            <button onClick={() => setShowCreate(false)} className="absolute top-4 right-4 text-dark-500 hover:text-dark-300">
              <X className="h-5 w-5" />
            </button>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button
                    ...
                    title="Delete"
                    onClick={(e) => { e.stopPropagation(); if (confirm(`Delete workspace "${w.name}"?`)) deleteMutation.mutate(w.workspace_id); }}
                  >
          ...
          {selectedWorkspace ? (
            <div className="space-y-6">
              ...
              <div className="flex gap-2 flex-wrap">
                ...
                <button className="btn-secondary text-sm" onClick={() => setShowImportScans(true)}>
              ...
              {summary && (() => {
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="card">
                    ...
                    <p className="text-2xl font-bold text-white">{stats.target_count ?? targetList.length}</p>
                  </div>
                  <div className="card">
                    <p className="text-sm text-dark-400">Scans</p>
                    <p className="text-2xl font-bold text-white">{stats.scan_count ?? 0}</p>
                  </div>
                  <div className="card">
                  ...
                  <div className="card">
                    ...
                    <p className="text-2xl font-bold text-white">{stats.correlation_count ?? 0}</p>
                  </div>
                </div>
                );
              })()}
                    ...
                    {targetList.map((t) => (
                      <div key={t.target_id} className="flex items-center justify-between p-3 bg-dark-700/50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-white text-sm font-mono">{t.value}</span>
              ...
              {/* Scans linked to this workspace */}
              <div className="card">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-semibold text-white">Linked Scans</h2>
                  <button
                ...
                {/* Scans associated via workspace detail (linked explicitly) */}
                {(() => {
                  const linkedScanIds = new Set(
                    (workspaceDetail?.scans as Array<{ scan_id: string }> ?? []).map((s) => s.scan_id)
                  );
                  // Also match scans whose target matches workspace targets (implicit)
                        ...
                        >
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              <p className="text-sm text-white truncate">{s.name || 'Untitled'}</p>
              ...
              {workspaceDetail && (
                <div className="card">
                  <h2 className="text-lg font-semibold text-white mb-3">Workspace Details</h2>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-dark-400">ID</p>
                      <p className="text-white font-mono text-xs">{workspaceDetail.workspace_id}</p>
                    </div>
                    <div>
                      <p className="text-dark-400">Created</p>
                      <p className="text-white">{formatEpoch(workspaceDetail.created_time)}</p>
                    </div>
                    {workspaceDetail.modified_time && (
                      <div>
                        <p className="text-dark-400">Last Modified</p>
                        <p className="text-white">{formatEpoch(workspaceDetail.modified_time)}</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Workspace AI Report */}
              {selectedWorkspace && <WorkspaceReportCard workspaceId={selectedWorkspace} workspace={workspaces.find((w) => w.workspace_id === selectedWorkspace)} summary={summary} />}
            </div>
          ) : (
      ...
      {showEdit && selectedWorkspace && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowEdit(false)} />
          <div className="relative bg-dark-800 border border-dark-700 rounded-2xl p-6 max-w-md w-full shadow-2xl animate-fade-in-up">
            <button onClick={() => setShowEdit(false)} className="absolute top-4 right-4 text-dark-500 hover:text-dark-300">
              <X className="h-5 w-5" />
            </button>
            <h2 className="text-xl font-bold text-white mb-4">Edit Workspace</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-sm text-dark-300 mb-1">Workspace Name</label>
      ...
      {showImportScans && selectedWorkspace && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowImportScans(false)} />
          <div className="relative bg-dark-800 border border-dark-700 rounded-2xl p-6 max-w-lg w-full shadow-2xl animate-fade-in-up">
            <button onClick={() => setShowImportScans(false)} className="absolute top-4 right-4 text-dark-500 hover:text-dark-300">
              <X className="h-5 w-5" />
            </button>
            <h2 className="text-xl font-bold text-white mb-4">Link Scans to Workspace</h2>
...
function WorkspaceReportCard({ workspaceId, workspace, summary }: {
  workspaceId: string;
  workspace?: Workspace;
  ...
  const renderSimpleMd = (md: string) => {
    return md.split('\n').map((line, i) => {
      ...
      if (hMatch) {
        ...
        const cls = lvl === 1 ? 'text-lg font-bold text-white mt-3 mb-1' : lvl === 2 ? 'text-base font-bold text-white mt-3 mb-1' : 'text-sm font-semibold text-white mt-2 mb-1';
        return <div key={i} className={cls} dangerouslySetInnerHTML={{ __html: inlineFmt(hMatch[2]) }} />;
      }
      if (/^---+$/.test(line.trim())) return <hr key={i} className="border-dark-700/50 my-3" />;
      if (line.trim().startsWith('>')) return <blockquote key={i} className="border-l-2 border-spider-500 pl-3 text-dark-400 italic text-sm my-1" dangerouslySetInnerHTML={{ __html: inlineFmt(line.replace(/^>\s*/, '')) }} />;
      const liMatch = line.match(/^(\d+\.|[-*])\s+(.*)/);
      if (liMatch) return <li key={i} className="text-sm text-dark-300 ml-4 list-disc" dangerouslySetInnerHTML={{ __html: inlineFmt(liMatch[2]) }} />;
      if (line.trim() === '') return <div key={i} className="h-1" />;
      return <p key={i} className="text-sm text-dark-300" dangerouslySetInnerHTML={{ __html: inlineFmt(line) }} />;
    });
  };
    ...
    <div className="card">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="btn-primary text-xs" onClick={() => generateMut.mutate()} disabled={generateMut.isPending}>
            {generateMut.isPending ? <><Loader2 className="h-3 w-3 animate-spin" /> Generating...</> : <><Sparkles className="h-3 w-3" /> AI Report</>}
          </button>
          {!reportContent && (
      ...
      {generateMut.isError && (
        <div className="flex items-center gap-2 mb-3 p-2 bg-yellow-900/10 border border-yellow-700/30 rounded-lg text-xs text-yellow-300">
          ...
          <span>AI unavailable. <button className="underline" onClick={generateClientReport}>Use quick report</button></span>
        </div>
      )}
