...
class sfp_checkphish(SpiderFootPlugin):
    meta = {
        ...
        "dataSource": {
            ...
            "references": ["https://checkphish.ai/docs/checkphish-api/"],
            "apiKeyInstructions": [
    ...
    def watchedEvents(self):
        return ["DOMAIN_NAME", "INTERNET_NAME", "URL_UNSCOPED", "URL_MALICIOUS"]

    # What events this module produces
    def producedEvents(self):
        return ["MALICIOUS_URL", "PHISHING_URL", "DOMAIN_REPUTATION", "RAW_RIR_DATA"]
    ...
    def submitScan(self, url):
        ...
        data = {"urlInfo": {"url": url}, "scanType": "full"}
    ...
    def getScanResults(self, job_id):
        ...
        data = {"jobID": job_id, "insights": True}
    ...
    def normalizeDomain(self, domain):
        ...
        if domain.lower().startswith("http://") or domain.lower().startswith(
            "https://"
        ):
            return domain
    ...
    def handleEvent(self, event):
        ...
        while retry_count < self.opts["max_retry"]:
            ...
            if status == "RUNNING":
                retry_count += 1
                self.debug(
                    f"Scan still running. Retrying in {self.opts['wait_time']} seconds (attempt {retry_count}/{self.opts['max_retry']})"
                )
                time.sleep(self.opts["wait_time"])
                continue
        ...
        if not result or result.get("status") != "DONE":
            self.error(
                f"Timed out waiting for CheckPhish results for {scan_url}")
            return
        ...
        if disposition == "phishing":
            ...
            phish_desc = (
                f"CheckPhish identified {scan_url} as a phishing site {confidence_str}"
            )
            if brand and brand != "unknown":
                phish_desc += f" targeting {brand}"

            evt = SpiderFootEvent(
                "PHISHING_URL", phish_desc, self.__name__, event)
            self.notifyListeners(evt)

            # Also report as generally malicious
            evt = SpiderFootEvent(
                "MALICIOUS_URL", phish_desc, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if eventName in ["DOMAIN_NAME", "INTERNET_NAME"]:
            ...
            evt = SpiderFootEvent("DOMAIN_REPUTATION",
                                  rep_desc, self.__name__, event)
            self.notifyListeners(evt)
