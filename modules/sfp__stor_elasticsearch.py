...
try:
    ...
    from elasticsearch.exceptions import AuthenticationException, ConnectionError, NotFoundError
    moduleEnabled = True
except ImportError:
    moduleEnabled = False
...
class sfp__stor_elasticsearch(SpiderFootPlugin):
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        if not moduleEnabled:
            self.error("Elasticsearch module dependencies missing. Install them with 'pip install elasticsearch'")
    ...
    def initializeElasticsearch(self):
        ...
        try:
            ...
            elif self.opts['elasticsearch_username'] and self.opts['elasticsearch_password']:
                es_args['basic_auth'] = (self.opts['elasticsearch_username'], self.opts['elasticsearch_password'])
    ...
    def create_index(self):
        ...
        try:
            if not self.es_client.indices.exists(index=self.indexName):
                ...
                self.es_client.indices.create(index=self.indexName, body=mapping)
    ...
    def index_event(self, event):
        ...
        doc = {
            ...
            "sourceDataSource": event.sourceDataSource if hasattr(event, 'sourceDataSource') else None,
            "hash": event.hash
        }
    ...
    def process_bulk_data(self):
        ...
        try:
            ...
            self.debug(f"Indexed {success} events to Elasticsearch, {failed} failed")
        except Exception as e:
