        ...
        if eventName in ["IP_ADDRESS", "DOMAIN_NAME"]:
            ...
            if services:
                for service in services:
                    ...
                    if (
                        hostname and
                        eventName == "DOMAIN_NAME" and
                        self.getTarget().matches(hostname) and
                        hostname not in hosts
                    ):
                        if (
                            self.opts["verify"] and
                            not self.sf.resolveHost(hostname) and
                            not self.sf.resolveHost6(hostname)
                        ):
                            self.debug(
                    ...
                    if (
                        ip and
                        eventName != "IP_ADDRESS" and
                        self.sf.validIP(ip) and
                        ip not in ips
                    ):
                        evt = SpiderFootEvent(
            ...
            if leaks:
                for leak in leaks:
                    ...
                    if (
                        leak_protocol == "web" and
                        hostname and
                        not self.sf.validIP(hostname) and
                        not self.getTarget().matches(hostname)
                    ):
                        continue
