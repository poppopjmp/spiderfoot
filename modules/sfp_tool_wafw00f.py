...
class sfp_tool_wafw00f(SpiderFootPlugin):
    ...
    opts = {"pythonbinary": "python3", "wafw00f_path": ""}
    ...
    def handleEvent(self, event):
        ...
        if not SpiderFootHelpers.sanitiseInput(eventData):
            self.error(
                f"Invalid input, refusing to run {tool_name}: {eventData}")
            return

        args = [pythonBinary, tool_path, "-a", "-v", eventData]
        ...
        try:
            ...
            self.error(
                f"{tool_name} took too long to complete for {eventData}")
            return
        ...
        if p.returncode != 0:
            self.error(
                f"{tool_name} failed to run: {stderr.decode('utf-8').strip() if stderr else ''}"
            )
            return
        ...
        if stdout:
            ...
            if found_waf and firewall_name:
                evt = SpiderFootEvent(
                    "RAW_RIR_DATA",
                    f"{tool_name} detected: {firewall_name}",
                    self.__name__,
                    event,
                )
                ...
                evt = SpiderFootEvent(
                    "WEBSERVER_TECHNOLOGY", f"WAF: {tech}", self.__name__, event
                )
                self.notifyListeners(evt)
            else:
            ...
            self.debug(
                f"{tool_name} did not produce any output for {eventData}")
