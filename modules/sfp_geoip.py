...
class sfp_geoip(SpiderFootPlugin):
    ...
    def queryIPInfo(self, ip):
        ...
        if not self.opts['api_key'] and not self.opts['local_cache_only']:
            self.error("You enabled sfp_geoip but did not set an API key and local_cache_only is False!")
        ...
        try:
            ...
            self.sf.cachePut("geoip_" + ip, res['content'], 86400)  # Cache for 24 hours
            return data
    ...
    def processNetblock(self, netblock):
        ...
        try:
            ...
            if net.prefixlen < self.opts['max_subnets']:
                self.debug(f"Netblock {netblock} too large to process completely")
                
                # Break it down into smaller subnets
                subnets = list(net.subnets(new_prefix=self.opts['max_subnets']))
                
                # Limit the number of subnets to process
                if len(subnets) > self.opts['max_netblocks']:
                    self.debug(f"Limiting netblock analysis to {self.opts['max_netblocks']} subnets")
                ...
                for subnet in subnets:
                    ...
                    if subnet_size > 0:
                        ...
                        if not self.opts['skip_private_ips'] or not self.isPrivateIP(first_ip):
                ...
                for ip in ips:
                    ip_str = str(ip)
                    if not self.opts['skip_private_ips'] or not self.isPrivateIP(ip_str):
    ...
    def handleEvent(self, event):
        ...
        if location:
            evt = SpiderFootEvent("GEOINFO", ", ".join(location), self.__name__, event)
            self.notifyListeners(evt)
        ...
        if address_parts:
            ...
            evt = SpiderFootEvent("PHYSICAL_ADDRESS", address, self.__name__, event)
            self.notifyListeners(evt)
        ...
        if data.get('org'):
            ...
            if ' ' in org and org.split(' ')[0].startswith('AS'):
                ...
                evt = SpiderFootEvent("PROVIDER_TELCO", company, self.__name__, event)
                self.notifyListeners(evt)
            else:
                evt = SpiderFootEvent("COMPANY_NAME", org, self.__name__, event)
                self.notifyListeners(evt)
                
                # Also report as provider
                evt = SpiderFootEvent("PROVIDER_TELCO", org, self.__name__, event)
                self.notifyListeners(evt)
