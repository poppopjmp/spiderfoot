...
class sfp_fringeproject(SpiderFootPlugin):
    ...
    def query(self, qry, endpoint="search"):
        ...
        if not res or not res.get('content'):
            self.debug(f"No results found from Fringe Project for {qry} ({endpoint})")
            return None
    ...
    def handleEvent(self, event):
        ...
        if eventName == "DOMAIN_NAME" or eventName == "INTERNET_NAME":
            ...
            for subdomain in subdomains:
                if self.opts['verify']:
                    if self.sf.resolveHost(subdomain) or self.sf.resolveHost('www.' + subdomain):
                        evt = SpiderFootEvent("INTERNET_NAME", subdomain, self.__name__, event)
                        self.notifyListeners(evt)
                        
                        if self.sf.isDomain(subdomain, self.opts['_internettlds']):
                            evt = SpiderFootEvent("DOMAIN_NAME", subdomain, self.__name__, event)
                            self.notifyListeners(evt)
                    else:
            ...
            if webserver:
                if webserver.get('banner'):
                    evt = SpiderFootEvent("WEBSERVER_BANNER", webserver.get('banner'), self.__name__, event)
                    self.notifyListeners(evt)
                ...
                if headers:
                    evt = SpiderFootEvent("WEBSERVER_HTTPHEADERS", json.dumps(headers), self.__name__, event)
                    self.notifyListeners(evt)
                    
                    # Check for server software info in headers
                    if headers.get('Server'):
                        evt = SpiderFootEvent("SOFTWARE_USED", f"Web Server: {headers.get('Server')}", self.__name__, event)
                        self.notifyListeners(evt)
                    ...
                    if result_type == 'domain':
                        if self.opts['verify']:
                            if self.sf.resolveHost(result_value):
                                evt = SpiderFootEvent("INTERNET_NAME", result_value, self.__name__, event)
                                self.notifyListeners(evt)
                                
                                if self.sf.isDomain(result_value, self.opts['_internettlds']):
                                    evt = SpiderFootEvent("DOMAIN_NAME", result_value, self.__name__, event)
                                    self.notifyListeners(evt)
                            else:
                                evt = SpiderFootEvent("INTERNET_NAME_UNRESOLVED", result_value, self.__name__, event)
                                self.notifyListeners(evt)
                        else:
                            evt = SpiderFootEvent("INTERNET_NAME", result_value, self.__name__, event)
                            self.notifyListeners(evt)
                        ...
                        evt = SpiderFootEvent("IP_ADDRESS", result_value, self.__name__, event)
                        self.notifyListeners(evt)
