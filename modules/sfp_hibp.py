...
class sfp_hibp(SpiderFootPlugin):
    ...
    def queryHibp(self, email, check_type="breachedaccount"):
        ...
        if res["code"] == "429":
            ...
            # Sleep and don't retry on rate limit
        ...
        if res["code"] != "200":
            self.error(
                f"Unexpected HTTP response code {res['code']} from Have I Been Pwned"
            )
            self.errorState = True
            return None
        ...
        try:
            ...
            self.error(
                f"Error processing JSON response from Have I Been Pwned: {e}")
            return None
    ...
    def queryDomainBreaches(self, domain):
        ...
        # Parse the response
        if not res["content"]:
            self.debug("No domain breach data received from Have I Been Pwned")
            return []
    ...
    def handleEvent(self, event):
                ...
                if len(breaches) > 0:
                    ...
                    for breach in breaches:
                        ...
                        if self.opts["max_age_days"] > 0 and breach_date != "Unknown":
                            try:
                                breach_date_obj = datetime.strptime(
                                    breach_date, "%Y-%m-%d"
                                )
                                age_days = (datetime.now() -
                                            breach_date_obj).days
                                if age_days > self.opts["max_age_days"]:
                                    self.debug(
                                        f"Skipping breach {breach_name} due to age ({age_days} days)"
                                    )
                                    continue
                            except Exception:
                        ...
                        if breach.get("DataClasses") and "Passwords" in breach.get(
                            "DataClasses"
                        ):
                            evt = SpiderFootEvent(
                                "PASSWORD_COMPROMISED",
                        ...
                        evt = SpiderFootEvent(
                            "LEAKSITE_CONTENT", content, self.__name__, event
                        )
                        self.notifyListeners(evt)
                ...
                if len(pastes) > 0:
                    ...
                    for paste in pastes:
                        ...
                        if paste.get("Source") and paste.get("Id"):
                            ...
                            evt = SpiderFootEvent(
                                "LEAKSITE_URL", url, self.__name__, event
                            )
                            self.notifyListeners(evt)
        ...
        # Handle domain names
            ...
            if breach_count > 0:
                ...
                if (
                    self.opts["max_domain_breaches"] > 0 and
                    breach_count > self.opts["max_domain_breaches"]
                ):
                    domain_breaches = domain_breaches[
                        : self.opts["max_domain_breaches"]
                    ]

                # Create raw RIR data event for domain breaches
                evt = SpiderFootEvent(
                    "RAW_RIR_DATA",
                    json.dumps({"type": "domain_breaches",
                               "data": domain_breaches}),
                    self.__name__,
                    event,
                )
                self.notifyListeners(evt)
