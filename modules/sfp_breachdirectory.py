...
class sfp_breachdirectory(SpiderFootPlugin):
    ...
    def query(self, qry, query_type="email"):
        ...
        if not self.opts['api_key']:
            self.error("You enabled sfp_breachdirectory but did not set an API key!")
            return None
        ...
        if res['code'] != "200":
            self.error(f"Unexpected HTTP response code {res['code']} from BreachDirectory API.")
            return None
        ...
        try:
            ...
            self.error(f"Error processing JSON response from BreachDirectory: {e}")
            return None
    ...
    def handleEvent(self, event):
        ...
        if eventName == "EMAILADDR":
            ...
            for breach in breaches:
                ...
                evt = SpiderFootEvent("EMAILADDR_COMPROMISED", desc, self.__name__, event)
                self.notifyListeners(evt)
            ...
            data = self.query(eventData, "username" if eventName == "USERNAME" else "phone")
            
            if not data:
                return
                
            if data.get("result") == "null":
                self.debug(f"No breach data found for {eventName}: {eventData}")
                return
            ...
            evt = SpiderFootEvent("DOMAIN_BREACHED", desc, self.__name__, event)
            self.notifyListeners(evt)
