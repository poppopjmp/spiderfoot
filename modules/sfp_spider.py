...
class sfp_spider(SpiderFootPlugin):
    ...
    def processUrl(self, url: str) -> dict:
        ...
        if list(
            filter(
                lambda ext: url.lower().split(
                    "?")[0].endswith("." + ext.lower()),
    ...
    def cleanLinks(self, links: list) -> list:
        ...
        for link in links:
            ...
            if linkBase in self.robotsRules and self.opts["robotsonly"]:
                if list(
                    filter(
                        lambda blocked: type(blocked).lower(blocked) in link.lower() or
                        blocked == "*",
    ...
    def contentNotify(self, url: str, httpresult: dict, parentEvent=None) -> None:
        ...
        if store_content:
            ...
            if content:
                event = SpiderFootEvent(
                    "TARGET_WEB_CONTENT", str(
                        content), self.__name__, parentEvent
                )
                event.actualSource = url
                self.notifyListeners(event)
                ...
                if res:
                    ...
                    if robots_txt:
                        ...
                        self.robotsRules[targetBase] = (
                            SpiderFootHelpers.extractUrlsFromRobotsTxt(
                                robots_txt)
                        )
