...
class sfp_dnssublist(SpiderFootPlugin):
    ...
    COMMON_SUBDOMAINS = [
        ...
        "blog", "dev", "test", "portal", "ns1", "ns2", "smtp", "secure",
        "vpn", "m", "cloud", "api", "cdn", "app", "exchange", "owa",
        "intranet", "remote", "server", "cpanel", "webdisk", "ssh", "mx"
    ]
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        if self.opts['wordlist'] != "default" and self.opts['bruteforce']:
            try:
                ...
                self.debug(f"Loaded {len(self.COMMON_SUBDOMAINS)} subdomains from wordlist")
                ...
                if zone:
                    ...
                    for name, node in zone.nodes.items():
                        ...
                        if record_name.endswith(f".{domain}") or record_name == domain:
                            self.subdomains.add(record_name.rstrip('.').lower())
        ...
        for subdomain in self.subdomains:
            ...
            if self.opts['validate']:
                ...
                if is_valid:
                    ...
                    evt = SpiderFootEvent("INTERNET_NAME", subdomain, self.__name__, parentEvent)
                    self.notifyListeners(evt)
                    
                    # Check if it's a domain name
                    if self.sf.isDomain(subdomain, self.opts['_internettlds']):
                        evt = SpiderFootEvent("DOMAIN_NAME", subdomain, self.__name__, parentEvent)
                        self.notifyListeners(evt)
                        
                    # Create IP address events
                    for ip in ips:
                        evt = SpiderFootEvent("IP_ADDRESS", ip, self.__name__, evt)
                        self.notifyListeners(evt)
                        
                    for ipv6 in ipv6s:
                        evt = SpiderFootEvent("IPV6_ADDRESS", ipv6, self.__name__, evt)
                        self.notifyListeners(evt)
                else:
                    # Create unresolved event
                    evt = SpiderFootEvent("INTERNET_NAME_UNRESOLVED", subdomain, self.__name__, parentEvent)
                    self.notifyListeners(evt)
            else:
