...
def update_module_file(filepath, dry_run=False, verbose=False):
    ...
    if re.search(import_pattern, content):
        content = re.sub(import_pattern, '# Module now uses the logging from the SpiderFootPlugin base class', content)
        if verbose:
            print(f"  - Removed logconfig import")
    ...
    for pattern, replacement in patterns:
        ...
        if matches:
            ...
            if verbose:
                print(f"  - Replaced {len(matches)} instances of {pattern.split('(')[0]} with {replacement.split('(')[0]}")
...
def main():
    parser = argparse.ArgumentParser(description="Migrate SpiderFoot modules to the new logging system")
    parser.add_argument("--dry-run", action="store_true", help="Don't write any changes to files")
    parser.add_argument("--verbose", "-v", action="store_true", help="Print detailed information")
    parser.add_argument("--module", help="Process only a specific module (e.g., sfp_example)")
    parser.add_argument("--dir", "-d", help="Directory containing SpiderFoot modules")
    ...
    if args.dir:
        ...
        if not os.path.isdir(modules_dir):
            ...
            for potential_dir in potential_dirs:
                ...
                print("Please specify the directory with --dir or run from the SpiderFoot root directory.")
                sys.exit(1)
    ...
    if args.module:
        ...
        print(f"Summary: {updated}/{total} modules {'would be' if args.dry_run else 'were'} updated.")
