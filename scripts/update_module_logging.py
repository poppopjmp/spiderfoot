...
def update_module_logging(file_path, dry_run=False):
    ...
    if debug_count > 0:
        ...
        changes.append(f"Replaced {debug_count} log.debug calls with self.debug")
    ...
    if error_count > 0:
        ...
        changes.append(f"Replaced {error_count} log.error calls with self.error")
    ...
    if warning_count > 0:
        ...
        changes.append(f"Replaced {warning_count} log.warning calls with self.debug")
    
    # Handle cases where 'self.log' is used instead of 'log'
    self_log_count = len(re.findall(r'self\.log\.debug\(', content))
    if self_log_count > 0:
        content = re.sub(r'self\.log\.debug\((.*?)\)', r'self.debug(\1)', content)
        changes.append(f"Replaced {self_log_count} self.log.debug calls with self.debug")
    
    self_log_info_count = len(re.findall(r'self\.log\.info\(', content))
    if self_log_info_count > 0:
        content = re.sub(r'self\.log\.info\((.*?)\)', r'self.info(\1)', content)
        changes.append(f"Replaced {self_log_info_count} self.log.info calls with self.info")
    
    self_log_error_count = len(re.findall(r'self\.log\.error\(', content))
    if self_log_error_count > 0:
        content = re.sub(r'self\.log\.error\((.*?)\)', r'self.error(\1)', content)
        changes.append(f"Replaced {self_log_error_count} self.log.error calls with self.error")
...
def main():
    parser = argparse.ArgumentParser(description="Update SpiderFoot modules to use the new logging system")
    parser.add_argument("--dry-run", action="store_true", help="Don't modify files, just show what would be done")
    parser.add_argument("--module", help="Update specific module (e.g., sfp_example)")
