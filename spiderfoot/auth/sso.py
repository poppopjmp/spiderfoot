...
class SSOManager:
    ...
    def process_saml_response(
        ...
        if not os.environ.get("SF_SSO_DEV_MODE", "").lower() in ("true", "1", "yes"):
            _log.error(
                "SAML response processing requires a production SAML library. "
                "Set SF_SSO_DEV_MODE=true to use development stubs (NEVER in production)."
            )
            return {"error": "SAML response processing not available — production SAML library required"}
        _log.critical(
            "SSO DEV MODE ACTIVE — SAML response NOT validated. "
            "Do NOT use SF_SSO_DEV_MODE=true in production."
        )
    ...
    def process_oidc_callback(
        ...
        if not os.environ.get("SF_SSO_DEV_MODE", "").lower() in ("true", "1", "yes"):
            _log.error(
                "OIDC token exchange requires a production OIDC library. "
                "Set SF_SSO_DEV_MODE=true to use development stubs (NEVER in production)."
            )
            return {"error": "OIDC token exchange not available — production library required"}
        _log.critical(
            "SSO DEV MODE ACTIVE — OIDC code NOT exchanged with IdP. "
            "Do NOT use SF_SSO_DEV_MODE=true in production."
        )
