...
Usage::
    ...
    from spiderfoot.observability.logging_config import configure_logging
...
_SENSITIVE_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    # API keys (various formats: sf_key_xxx, sk-xxx, key-xxx, etc.)
    (re.compile(r"\b((?:sf_key_|sk-|api[_-]?key[=:\s]+)[A-Za-z0-9_\-]{4})[A-Za-z0-9_\-]+", re.IGNORECASE), r"\1***REDACTED***"),
    # Bearer tokens
    (re.compile(r"(Bearer\s+[A-Za-z0-9_\-]{4})[A-Za-z0-9_\-.]+", re.IGNORECASE), r"\1***REDACTED***"),
    # Authorization header values
    (re.compile(r"([Aa]uthorization[=:\s]+\S{4})\S+"), r"\1***REDACTED***"),
    ...
    # Generic secret/token in key=value patterns
    (re.compile(r"((?:secret|token|api_key|apikey|access_key|private_key)[\"']?\s*[:=]\s*[\"']?)[^\"',\s}]{4,}", re.IGNORECASE), r"\1***REDACTED***"),
]


class SensitiveDataFilter(logging.Filter):
    """Logging filter that redacts secrets, tokens, keys, and passwords.

    Attach to any handler or logger to automatically scrub sensitive
    data before it reaches log output.
    """
...
def configure_logging(
    ...
    *,
    ...
    # ── Sensitive data filter (applied to root so all handlers benefit) ──
    scrub_filter = SensitiveDataFilter()
...
def _add_vector_handler(logger: logging.Logger, config: dict) -> None:
    ...
    try:
        from spiderfoot.observability.vector_sink import VectorConfig, VectorLogHandler, VectorSink
