...
# Query parameters that must be redacted in logs
...
# Regex to match query parameters: key=value pairs in URLs
_QUERY_PARAM_RE = re.compile(
    r"([?&])(" + "|".join(re.escape(p) for p in _SENSITIVE_PARAMS) + r")=([^&]*)",
    re.IGNORECASE,
)
...
def scrub_url(url: str) -> str:
    ...
    Example::
        ...
        scrub_url("/api/scan?api_key=secret123&target=example.com")
        # → "/api/scan?api_key=***&target=example.com"
    """
    return _QUERY_PARAM_RE.sub(r"\1\2=***", url)


# Regex for valid request IDs: alphanumeric, hyphens, underscores, dots, max 128 chars
...
def _sanitize_request_id(raw: str | None) -> str | None:
    ...
    Returns the ID if it matches the allowed pattern, otherwise ``None``
    (which causes the middleware to generate a new ID instead).

    This prevents log-injection attacks via crafted ``X-Request-ID``
    headers containing newlines, control characters, or excessive length.
    """
    if raw is None:
        return None
...
if HAS_STARLETTE:
    ...
    class RequestTracingMiddleware(BaseHTTPMiddleware):
        ...
        async def dispatch(self, request: Request, call_next: Callable) -> Response:
            ...
            client_id = _sanitize_request_id(
