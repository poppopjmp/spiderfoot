...
# NOTE: declared before /scans/{scan_id} so the parameterized route (scan_id is
# a SafeId, which also matches the literal "compare") does not shadow it.
# Starlette matches routes in declaration order.
