...
log = logging.getLogger("spiderfoot.security.rate_limiter")
...
class RateLimiterService:
    ...
    def get_limit(self, key: str) -> RateLimit:
        ...
        Supports hierarchical keys: if *key* is ``"client:api"`` and no
        exact match exists, the suffix after the last colon (``"api"``) is
        tried before falling back to the global default.  This lets callers
        use ``set_limit("api", ...)`` while checking with per-client keys.
        """
        if key in self._limits:
            return self._limits[key]
        # Try suffix after last colon (e.g. "192.168.1.1:api" → "api")
        parts = key.rsplit(':', 1)
        if len(parts) == 2 and parts[1] in self._limits:
            return self._limits[parts[1]]
