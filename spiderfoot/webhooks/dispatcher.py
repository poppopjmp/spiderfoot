...
Usage::
    ...
    from spiderfoot.webhooks.dispatcher import WebhookDispatcher, WebhookConfig
...
def _is_ssrf_target(url: str) -> bool:
    ...
    Blocks RFC 1918, RFC 6598, loopback, link-local, and cloud metadata
    endpoints.  Performs DNS resolution to catch DNS rebinding through
...
class WebhookDispatcher:
    ...
    def deliver(
        ...
        # SSRF guard — block delivery to internal/private endpoints
        if _is_ssrf_target(config.url):
            log.warning(
                "Webhook blocked (SSRF): %s -> %s",
                event_type, config.url,
            )
            record = DeliveryRecord(
                ...
                error="URL blocked: resolves to internal/private address",
