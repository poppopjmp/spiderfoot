...
class TestSf(unittest.TestCase):
    ...
    def test_no_args_should_print_arg_l_required(self):
        ...
        self.assertIn(
            b"SpiderFoot requires -l <ip>:<port> to start the web server. Try --help for guidance.", out)
    ...
    def test_run_scan_invalid_target_should_exit(self):
        ...
        self.assertIn(bytes(
            f"Could not determine target type. Invalid target: {invalid_target}", 'utf-8'), err)
        self.assertEqual(255, code)
