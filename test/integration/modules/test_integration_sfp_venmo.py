# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_venmo.py
import pytest
import os
...
# This test requires credentials for the Venmo service
# To run this test, set the environment variables:
# - SF_SFP_VENMO_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_VENMO_API_KEY"]),
    reason="Integration test - requires Venmo credentials",
)
class TestModuleIntegrationVenmo:
    """Integration testing for the Venmo module."""

    @pytest.fixture
    def module(self):
        """Return a Venmo module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
