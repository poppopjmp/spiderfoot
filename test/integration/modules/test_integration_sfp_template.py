# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_template.py
import pytest
...
# This test requires credentials for the Template service
# To run this test, set the environment variables:
# - SF_SFP_TEMPLATE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TEMPLATE_API_KEY']),
    reason="Integration test - requires Template credentials"
)
class TestModuleIntegrationTemplate:
    """Integration testing for the Template module."""

    @pytest.fixture
    def module(self):
        """Return a Template module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
