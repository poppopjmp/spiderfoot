# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_junkfiles.py
import pytest
...
# This test requires credentials for the Junkfiles service
# To run this test, set the environment variables:
# - SF_SFP_JUNKFILES_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_JUNKFILES_API_KEY']),
    reason="Integration test - requires Junkfiles credentials"
)
class TestModuleIntegrationJunkfiles:
    """Integration testing for the Junkfiles module."""

    @pytest.fixture
    def module(self):
        """Return a Junkfiles module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
