# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_neutrinoapi.py
import pytest
...
# This test requires credentials for the Neutrinoapi service
# To run this test, set the environment variables:
# - SF_SFP_NEUTRINOAPI_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_NEUTRINOAPI_API_KEY']),
    reason="Integration test - requires Neutrinoapi credentials"
)
class TestModuleIntegrationNeutrinoapi:
    """Integration testing for the Neutrinoapi module."""

    @pytest.fixture
    def module(self):
        """Return a Neutrinoapi module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
