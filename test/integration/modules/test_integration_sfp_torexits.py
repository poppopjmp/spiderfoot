# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_torexits.py
import pytest
...
# This test requires credentials for the Torexits service
# To run this test, set the environment variables:
# - SF_SFP_TOREXITS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TOREXITS_API_KEY']),
    reason="Integration test - requires Torexits credentials"
)
class TestModuleIntegrationTorexits:
    """Integration testing for the Torexits module."""

    @pytest.fixture
    def module(self):
        """Return a Torexits module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
