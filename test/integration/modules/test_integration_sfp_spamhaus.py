# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_spamhaus.py
import pytest
...
# This test requires credentials for the Spamhaus service
# To run this test, set the environment variables:
# - SF_SFP_SPAMHAUS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SPAMHAUS_API_KEY']),
    reason="Integration test - requires Spamhaus credentials"
)
class TestModuleIntegrationSpamhaus:
    """Integration testing for the Spamhaus module."""

    @pytest.fixture
    def module(self):
        """Return a Spamhaus module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
