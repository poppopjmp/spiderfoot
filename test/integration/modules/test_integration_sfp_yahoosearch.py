# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_yahoosearch.py
import pytest
...
# This test requires credentials for the Yahoosearch service
# To run this test, set the environment variables:
# - SF_SFP_YAHOOSEARCH_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_YAHOOSEARCH_API_KEY']),
    reason="Integration test - requires Yahoosearch credentials"
)
class TestModuleIntegrationYahoosearch:
    """Integration testing for the Yahoosearch module."""

    @pytest.fixture
    def module(self):
        """Return a Yahoosearch module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
