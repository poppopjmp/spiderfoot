# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_crxcavator.py
import pytest
...
# This test requires credentials for the Crxcavator service
# To run this test, set the environment variables:
# - SF_SFP_CRXCAVATOR_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CRXCAVATOR_API_KEY']),
    reason="Integration test - requires Crxcavator credentials"
)
class TestModuleIntegrationCrxcavator:
    """Integration testing for the Crxcavator module."""

    @pytest.fixture
    def module(self):
        """Return a Crxcavator module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
