# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_misp.py
import pytest
...
# This test requires credentials for the Misp service
# To run this test, set the environment variables:
# - SF_SFP_MISP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_MISP_API_KEY']),
    reason="Integration test - requires Misp credentials"
)
class TestModuleIntegrationMisp:
    """Integration testing for the Misp module."""

    @pytest.fixture
    def module(self):
        """Return a Misp module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
