# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dronebl.py
import pytest
import os
...
# This test requires credentials for the Dronebl service
# To run this test, set the environment variables:
# - SF_SFP_DRONEBL_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_DRONEBL_API_KEY"]),
    reason="Integration test - requires Dronebl credentials",
)
class TestModuleIntegrationDronebl:
    """Integration testing for the Dronebl module."""

    @pytest.fixture
    def module(self):
        """Return a Dronebl module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
