# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_xforce.py
import pytest
...
# This test requires credentials for the X Force service
# To run this test, set the environment variables:
# - SF_SFP_XFORCE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_XFORCE_API_KEY']),
    reason="Integration test - requires X Force credentials"
)
class TestModuleIntegrationxForce:
    """Integration testing for the X Force module."""

    @pytest.fixture
    def module(self):
        """Return a X Force module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
