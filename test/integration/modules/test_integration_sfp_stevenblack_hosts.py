# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_stevenblack_hosts.py
import pytest
...
# This test requires credentials for the Stevenblack Hosts service
# To run this test, set the environment variables:
# - SF_SFP_STEVENBLACK_HOSTS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_STEVENBLACK_HOSTS_API_KEY']),
    reason="Integration test - requires Stevenblack Hosts credentials"
)
class TestModuleIntegrationStevenblackHosts:
    """Integration testing for the Stevenblack Hosts module."""

    @pytest.fixture
    def module(self):
        """Return a Stevenblack Hosts module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
