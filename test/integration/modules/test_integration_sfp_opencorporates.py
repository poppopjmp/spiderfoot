# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_opencorporates.py
import pytest
...
# This test requires credentials for the Opencorporates service
# To run this test, set the environment variables:
# - SF_SFP_OPENCORPORATES_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_OPENCORPORATES_API_KEY']),
    reason="Integration test - requires Opencorporates credentials"
)
class TestModuleIntegrationOpencorporates:
    """Integration testing for the Opencorporates module."""

    @pytest.fixture
    def module(self):
        """Return a Opencorporates module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
