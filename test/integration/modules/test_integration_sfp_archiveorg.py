# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_archiveorg.py
import pytest
...
# This test requires credentials for the Archiveorg service
# To run this test, set the environment variables:
# - SF_SFP_ARCHIVEORG_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ARCHIVEORG_API_KEY']),
    reason="Integration test - requires Archiveorg credentials"
)
class TestModuleIntegrationArchiveorg:
    """Integration testing for the Archiveorg module."""

    @pytest.fixture
    def module(self):
        """Return a Archiveorg module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
