# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_gravatar.py
import pytest
...
# This test requires credentials for the Gravatar service
# To run this test, set the environment variables:
# - SF_SFP_GRAVATAR_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_GRAVATAR_API_KEY']),
    reason="Integration test - requires Gravatar credentials"
)
class TestModuleIntegrationGravatar:
    """Integration testing for the Gravatar module."""

    @pytest.fixture
    def module(self):
        """Return a Gravatar module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
