# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_gleif.py
import pytest
import os
...
# This test requires credentials for the Gleif service
# To run this test, set the environment variables:
# - SF_SFP_GLEIF_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_GLEIF_API_KEY"]),
    reason="Integration test - requires Gleif credentials",
)
class TestModuleIntegrationGleif:
    """Integration testing for the Gleif module."""

    @pytest.fixture
    def module(self):
        """Return a Gleif module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
