# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnscommonsrv.py
import pytest
...
# This test requires credentials for the Dnscommonsrv service
# To run this test, set the environment variables:
# - SF_SFP_DNSCOMMONSRV_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSCOMMONSRV_API_KEY']),
    reason="Integration test - requires Dnscommonsrv credentials"
)
class TestModuleIntegrationDnscommonsrv:
    """Integration testing for the Dnscommonsrv module."""

    @pytest.fixture
    def module(self):
        """Return a Dnscommonsrv module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
