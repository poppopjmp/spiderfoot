# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_webanalytics.py
import pytest
...
# This test requires credentials for the Webanalytics service
# To run this test, set the environment variables:
# - SF_SFP_WEBANALYTICS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_WEBANALYTICS_API_KEY']),
    reason="Integration test - requires Webanalytics credentials"
)
class TestModuleIntegrationWebanalytics:
    """Integration testing for the Webanalytics module."""

    @pytest.fixture
    def module(self):
        """Return a Webanalytics module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
