# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_shodan.py
import pytest
import os
...
# This test requires credentials for the Shodan service
# To run this test, set the environment variables:
# - SF_SFP_SHODAN_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_SHODAN_API_KEY"]),
    reason="Integration test - requires Shodan credentials",
)
class TestModuleIntegrationShodan:
    """Integration testing for the Shodan module."""

    @pytest.fixture
    def module(self):
        """Return a Shodan module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
