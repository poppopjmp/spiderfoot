# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_censys.py
import pytest
...
# This test requires credentials for the Censys service
# To run this test, set the environment variables:
# - SF_SFP_CENSYS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CENSYS_API_KEY']),
    reason="Integration test - requires Censys credentials"
)
class TestModuleIntegrationCensys:
    """Integration testing for the Censys module."""

    @pytest.fixture
    def module(self):
        """Return a Censys module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
