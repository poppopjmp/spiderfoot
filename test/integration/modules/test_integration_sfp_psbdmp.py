# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_psbdmp.py
import pytest
import os
...
# This test requires credentials for the Psbdmp service
# To run this test, set the environment variables:
# - SF_SFP_PSBDMP_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_PSBDMP_API_KEY"]),
    reason="Integration test - requires Psbdmp credentials",
)
class TestModuleIntegrationPsbdmp:
    """Integration testing for the Psbdmp module."""

    @pytest.fixture
    def module(self):
        """Return a Psbdmp module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
