# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_crobat_api.py
import pytest
...
# This test requires credentials for the Crobat Api service
# To run this test, set the environment variables:
# - SF_SFP_CROBAT_API_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CROBAT_API_API_KEY']),
    reason="Integration test - requires Crobat Api credentials"
)
class TestModuleIntegrationCrobatApi:
    """Integration testing for the Crobat Api module."""

    @pytest.fixture
    def module(self):
        """Return a Crobat Api module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
