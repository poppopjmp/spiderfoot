# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_threatcrowd.py
import pytest
...
# This test requires credentials for the Threatcrowd service
# To run this test, set the environment variables:
# - SF_SFP_THREATCROWD_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_THREATCROWD_API_KEY']),
    reason="Integration test - requires Threatcrowd credentials"
)
class TestModuleIntegrationThreatcrowd:
    """Integration testing for the Threatcrowd module."""

    @pytest.fixture
    def module(self):
        """Return a Threatcrowd module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
