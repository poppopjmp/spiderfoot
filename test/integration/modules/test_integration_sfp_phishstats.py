# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_phishstats.py
import pytest
...
# This test requires credentials for the Phishstats service
# To run this test, set the environment variables:
# - SF_SFP_PHISHSTATS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_PHISHSTATS_API_KEY']),
    reason="Integration test - requires Phishstats credentials"
)
class TestModuleIntegrationPhishstats:
    """Integration testing for the Phishstats module."""

    @pytest.fixture
    def module(self):
        """Return a Phishstats module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
