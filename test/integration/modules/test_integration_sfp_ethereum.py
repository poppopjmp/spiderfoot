# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_ethereum.py
import pytest
...
# This test requires credentials for the Ethereum service
# To run this test, set the environment variables:
# - SF_SFP_ETHEREUM_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ETHEREUM_API_KEY']),
    reason="Integration test - requires Ethereum credentials"
)
class TestModuleIntegrationEthereum:
    """Integration testing for the Ethereum module."""

    @pytest.fixture
    def module(self):
        """Return a Ethereum module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
