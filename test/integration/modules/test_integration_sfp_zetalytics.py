# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_zetalytics.py
import pytest
...
# This test requires credentials for the Zetalytics service
# To run this test, set the environment variables:
# - SF_SFP_ZETALYTICS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ZETALYTICS_API_KEY']),
    reason="Integration test - requires Zetalytics credentials"
)
class TestModuleIntegrationZetalytics:
    """Integration testing for the Zetalytics module."""

    @pytest.fixture
    def module(self):
        """Return a Zetalytics module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
