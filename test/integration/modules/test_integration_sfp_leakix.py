# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_leakix.py
import pytest
...
# This test requires credentials for the Leakix service
# To run this test, set the environment variables:
# - SF_SFP_LEAKIX_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_LEAKIX_API_KEY']),
    reason="Integration test - requires Leakix credentials"
)
class TestModuleIntegrationLeakix:
    """Integration testing for the Leakix module."""

    @pytest.fixture
    def module(self):
        """Return a Leakix module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
