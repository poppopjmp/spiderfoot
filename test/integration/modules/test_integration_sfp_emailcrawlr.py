# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_emailcrawlr.py
import pytest
...
# This test requires credentials for the Emailcrawlr service
# To run this test, set the environment variables:
# - SF_SFP_EMAILCRAWLR_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_EMAILCRAWLR_API_KEY']),
    reason="Integration test - requires Emailcrawlr credentials"
)
class TestModuleIntegrationEmailcrawlr:
    """Integration testing for the Emailcrawlr module."""

    @pytest.fixture
    def module(self):
        """Return a Emailcrawlr module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
