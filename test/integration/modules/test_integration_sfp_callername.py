# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_callername.py
import pytest
...
# This test requires credentials for the Callername service
# To run this test, set the environment variables:
# - SF_SFP_CALLERNAME_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CALLERNAME_API_KEY']),
    reason="Integration test - requires Callername credentials"
)
class TestModuleIntegrationCallername:
    """Integration testing for the Callername module."""

    @pytest.fixture
    def module(self):
        """Return a Callername module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
