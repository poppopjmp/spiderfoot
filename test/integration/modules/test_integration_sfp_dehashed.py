# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dehashed.py
import pytest
...
# This test requires credentials for the Dehashed service
# To run this test, set the environment variables:
# - SF_SFP_DEHASHED_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DEHASHED_API_KEY']),
    reason="Integration test - requires Dehashed credentials"
)
class TestModuleIntegrationDehashed:
    """Integration testing for the Dehashed module."""

    @pytest.fixture
    def module(self):
        """Return a Dehashed module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
