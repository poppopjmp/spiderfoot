# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_creditcard.py
import pytest
...
# This test requires credentials for the Creditcard service
# To run this test, set the environment variables:
# - SF_SFP_CREDITCARD_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CREDITCARD_API_KEY']),
    reason="Integration test - requires Creditcard credentials"
)
class TestModuleIntegrationCreditcard:
    """Integration testing for the Creditcard module."""

    @pytest.fixture
    def module(self):
        """Return a Creditcard module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
