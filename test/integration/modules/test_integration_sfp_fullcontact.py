# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_fullcontact.py
import pytest
...
# This test requires credentials for the Fullcontact service
# To run this test, set the environment variables:
# - SF_SFP_FULLCONTACT_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FULLCONTACT_API_KEY']),
    reason="Integration test - requires Fullcontact credentials"
)
class TestModuleIntegrationFullcontact:
    """Integration testing for the Fullcontact module."""

    @pytest.fixture
    def module(self):
        """Return a Fullcontact module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
