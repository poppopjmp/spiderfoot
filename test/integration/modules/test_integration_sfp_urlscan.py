# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_urlscan.py
import pytest
...
# This test requires credentials for the Urlscan service
# To run this test, set the environment variables:
# - SF_SFP_URLSCAN_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_URLSCAN_API_KEY']),
    reason="Integration test - requires Urlscan credentials"
)
class TestModuleIntegrationUrlscan:
    """Integration testing for the Urlscan module."""

    @pytest.fixture
    def module(self):
        """Return a Urlscan module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
