# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_reversewhois.py
import pytest
...
# This test requires credentials for the Reversewhois service
# To run this test, set the environment variables:
# - SF_SFP_REVERSEWHOIS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_REVERSEWHOIS_API_KEY']),
    reason="Integration test - requires Reversewhois credentials"
)
class TestModuleIntegrationReversewhois:
    """Integration testing for the Reversewhois module."""

    @pytest.fixture
    def module(self):
        """Return a Reversewhois module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
