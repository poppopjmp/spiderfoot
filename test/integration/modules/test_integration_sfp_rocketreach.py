# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_rocketreach.py
import pytest
...
# This test requires credentials for the Rocketreach service
# To run this test, set the environment variables:
# - SF_SFP_ROCKETREACH_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ROCKETREACH_API_KEY']),
    reason="Integration test - requires Rocketreach credentials"
)
class TestModuleIntegrationRocketreach:
    """Integration testing for the Rocketreach module."""

    @pytest.fixture
    def module(self):
        """Return a Rocketreach module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
