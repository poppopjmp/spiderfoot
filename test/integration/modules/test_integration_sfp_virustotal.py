# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_virustotal.py
import pytest
...
# This test requires credentials for the Virustotal service
# To run this test, set the environment variables:
# - SF_SFP_VIRUSTOTAL_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_VIRUSTOTAL_API_KEY']),
    reason="Integration test - requires Virustotal credentials"
)
class TestModuleIntegrationVirustotal:
    """Integration testing for the Virustotal module."""

    @pytest.fixture
    def module(self):
        """Return a Virustotal module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
