# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_cleantalk.py
import pytest
...
# This test requires credentials for the Cleantalk service
# To run this test, set the environment variables:
# - SF_SFP_CLEANTALK_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CLEANTALK_API_KEY']),
    reason="Integration test - requires Cleantalk credentials"
)
class TestModuleIntegrationCleantalk:
    """Integration testing for the Cleantalk module."""

    @pytest.fixture
    def module(self):
        """Return a Cleantalk module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
