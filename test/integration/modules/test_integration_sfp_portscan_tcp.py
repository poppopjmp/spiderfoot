# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_portscan_tcp.py
import pytest
...
# This test requires credentials for the Portscan Tcp service
# To run this test, set the environment variables:
# - SF_SFP_PORTSCAN_TCP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_PORTSCAN_TCP_API_KEY']),
    reason="Integration test - requires Portscan Tcp credentials"
)
class TestModuleIntegrationPortscanTcp:
    """Integration testing for the Portscan Tcp module."""

    @pytest.fixture
    def module(self):
        """Return a Portscan Tcp module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
