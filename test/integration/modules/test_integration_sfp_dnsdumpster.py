# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnsdumpster.py
import pytest
...
# This test requires credentials for the Dnsdumpster service
# To run this test, set the environment variables:
# - SF_SFP_DNSDUMPSTER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSDUMPSTER_API_KEY']),
    reason="Integration test - requires Dnsdumpster credentials"
)
class TestModuleIntegrationDnsdumpster:
    """Integration testing for the Dnsdumpster module."""

    @pytest.fixture
    def module(self):
        """Return a Dnsdumpster module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
