# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnszonexfer.py
import pytest
...
# This test requires credentials for the Dnszonexfer service
# To run this test, set the environment variables:
# - SF_SFP_DNSZONEXFER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSZONEXFER_API_KEY']),
    reason="Integration test - requires Dnszonexfer credentials"
)
class TestModuleIntegrationDnszonexfer:
    """Integration testing for the Dnszonexfer module."""

    @pytest.fixture
    def module(self):
        """Return a Dnszonexfer module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
