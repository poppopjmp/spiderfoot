# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_emergingthreats.py
import pytest
...
# This test requires credentials for the Emergingthreats service
# To run this test, set the environment variables:
# - SF_SFP_EMERGINGTHREATS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_EMERGINGTHREATS_API_KEY']),
    reason="Integration test - requires Emergingthreats credentials"
)
class TestModuleIntegrationEmergingthreats:
    """Integration testing for the Emergingthreats module."""

    @pytest.fixture
    def module(self):
        """Return a Emergingthreats module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
