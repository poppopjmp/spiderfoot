# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_zoneh.py
import pytest
...
# This test requires credentials for the Zoneh service
# To run this test, set the environment variables:
# - SF_SFP_ZONEH_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ZONEH_API_KEY']),
    reason="Integration test - requires Zoneh credentials"
)
class TestModuleIntegrationZoneh:
    """Integration testing for the Zoneh module."""

    @pytest.fixture
    def module(self):
        """Return a Zoneh module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
