# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_xref.py
import pytest
...
# This test requires credentials for the X Ref service
# To run this test, set the environment variables:
# - SF_SFP_XREF_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_XREF_API_KEY']),
    reason="Integration test - requires X Ref credentials"
)
class TestModuleIntegrationxRef:
    """Integration testing for the X Ref module."""

    @pytest.fixture
    def module(self):
        """Return a X Ref module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
