# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_darksearch.py
import pytest
...
# This test requires credentials for the Darksearch service
# To run this test, set the environment variables:
# - SF_SFP_DARKSEARCH_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DARKSEARCH_API_KEY']),
    reason="Integration test - requires Darksearch credentials"
)
class TestModuleIntegrationDarksearch:
    """Integration testing for the Darksearch module."""

    @pytest.fixture
    def module(self):
        """Return a Darksearch module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
