# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_seon.py
import pytest
...
# This test requires credentials for the Seon service
# To run this test, set the environment variables:
# - SF_SFP_SEON_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SEON_API_KEY']),
    reason="Integration test - requires Seon credentials"
)
class TestModuleIntegrationSeon:
    """Integration testing for the Seon module."""

    @pytest.fixture
    def module(self):
        """Return a Seon module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
