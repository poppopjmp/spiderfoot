# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_alienvaultiprep.py
import pytest
...
# This test requires credentials for the Alienvaultiprep service
# To run this test, set the environment variables:
# - SF_SFP_ALIENVAULTIPREP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ALIENVAULTIPREP_API_KEY']),
    reason="Integration test - requires Alienvaultiprep credentials"
)
class TestModuleIntegrationAlienvaultiprep:
    """Integration testing for the Alienvaultiprep module."""

    @pytest.fixture
    def module(self):
        """Return a Alienvaultiprep module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
