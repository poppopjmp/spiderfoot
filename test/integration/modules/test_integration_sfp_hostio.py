# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_hostio.py
import pytest
...
# This test requires credentials for the H Ostio service
# To run this test, set the environment variables:
# - SF_SFP_HOSTIO_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_HOSTIO_API_KEY']),
    reason="Integration test - requires H Ostio credentials"
)
class TestModuleIntegrationhOstio:
    """Integration testing for the H Ostio module."""

    @pytest.fixture
    def module(self):
        """Return a H Ostio module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
