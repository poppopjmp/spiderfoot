# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_clearbit.py
import pytest
...
# This test requires credentials for the Clearbit service
# To run this test, set the environment variables:
# - SF_SFP_CLEARBIT_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CLEARBIT_API_KEY']),
    reason="Integration test - requires Clearbit credentials"
)
class TestModuleIntegrationClearbit:
    """Integration testing for the Clearbit module."""

    @pytest.fixture
    def module(self):
        """Return a Clearbit module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
