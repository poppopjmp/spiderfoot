# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_commoncrawl.py
import pytest
...
# This test requires credentials for the Commoncrawl service
# To run this test, set the environment variables:
# - SF_SFP_COMMONCRAWL_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_COMMONCRAWL_API_KEY']),
    reason="Integration test - requires Commoncrawl credentials"
)
class TestModuleIntegrationCommoncrawl:
    """Integration testing for the Commoncrawl module."""

    @pytest.fixture
    def module(self):
        """Return a Commoncrawl module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
