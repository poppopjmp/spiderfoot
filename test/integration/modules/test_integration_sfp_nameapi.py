# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_nameapi.py
import pytest
...
# This test requires credentials for the Nameapi service
# To run this test, set the environment variables:
# - SF_SFP_NAMEAPI_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_NAMEAPI_API_KEY']),
    reason="Integration test - requires Nameapi credentials"
)
class TestModuleIntegrationNameapi:
    """Integration testing for the Nameapi module."""

    @pytest.fixture
    def module(self):
        """Return a Nameapi module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
