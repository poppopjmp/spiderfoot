# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_emailrep.py
import pytest
...
# This test requires credentials for the Emailrep service
# To run this test, set the environment variables:
# - SF_SFP_EMAILREP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_EMAILREP_API_KEY']),
    reason="Integration test - requires Emailrep credentials"
)
class TestModuleIntegrationEmailrep:
    """Integration testing for the Emailrep module."""

    @pytest.fixture
    def module(self):
        """Return a Emailrep module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
