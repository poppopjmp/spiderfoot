# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_accounts.py
import pytest
...
# This test requires credentials for the Accounts service
# To run this test, set the environment variables:
# - SF_SFP_ACCOUNTS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ACCOUNTS_API_KEY']),
    reason="Integration test - requires Accounts credentials"
)
class TestModuleIntegrationAccounts:
    """Integration testing for the Accounts module."""

    @pytest.fixture
    def module(self):
        """Return a Accounts module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
