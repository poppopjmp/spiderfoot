# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_haveibeenpwned.py
import pytest
...
# This test requires credentials for the H Aveibeenpwned service
# To run this test, set the environment variables:
# - SF_SFP_HAVEIBEENPWNED_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_HAVEIBEENPWNED_API_KEY']),
    reason="Integration test - requires H Aveibeenpwned credentials"
)
class TestModuleIntegrationhAveibeenpwned:
    """Integration testing for the H Aveibeenpwned module."""

    @pytest.fixture
    def module(self):
        """Return a H Aveibeenpwned module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
