# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_zonefiles.py
import pytest
...
# This test requires credentials for the Zonefiles service
# To run this test, set the environment variables:
# - SF_SFP_ZONEFILES_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ZONEFILES_API_KEY']),
    reason="Integration test - requires Zonefiles credentials"
)
class TestModuleIntegrationZonefiles:
    """Integration testing for the Zonefiles module."""

    @pytest.fixture
    def module(self):
        """Return a Zonefiles module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
