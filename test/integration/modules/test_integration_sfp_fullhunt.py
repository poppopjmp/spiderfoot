# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_fullhunt.py
import pytest
...
# This test requires credentials for the Fullhunt service
# To run this test, set the environment variables:
# - SF_SFP_FULLHUNT_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FULLHUNT_API_KEY']),
    reason="Integration test - requires Fullhunt credentials"
)
class TestModuleIntegrationFullhunt:
    """Integration testing for the Fullhunt module."""

    @pytest.fixture
    def module(self):
        """Return a Fullhunt module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
