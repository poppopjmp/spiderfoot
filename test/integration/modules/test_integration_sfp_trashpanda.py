# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_trashpanda.py
import pytest
...
# This test requires credentials for the Trashpanda service
# To run this test, set the environment variables:
# - SF_SFP_TRASHPANDA_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TRASHPANDA_API_KEY']),
    reason="Integration test - requires Trashpanda credentials"
)
class TestModuleIntegrationTrashpanda:
    """Integration testing for the Trashpanda module."""

    @pytest.fixture
    def module(self):
        """Return a Trashpanda module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
