# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_abstractapi.py
import pytest
...
# This test requires credentials for the Abstractapi service
# To run this test, set the environment variables:
# - SF_SFP_ABSTRACTAPI_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ABSTRACTAPI_API_KEY']),
    reason="Integration test - requires Abstractapi credentials"
)
class TestModuleIntegrationAbstractapi:
    """Integration testing for the Abstractapi module."""

    @pytest.fixture
    def module(self):
        """Return a Abstractapi module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
