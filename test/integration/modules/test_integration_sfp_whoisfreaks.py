# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_whoisfreaks.py
import pytest
...
# This test requires credentials for the Whoisfreaks service
# To run this test, set the environment variables:
# - SF_SFP_WHOISFREAKS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_WHOISFREAKS_API_KEY']),
    reason="Integration test - requires Whoisfreaks credentials"
)
class TestModuleIntegrationWhoisfreaks:
    """Integration testing for the Whoisfreaks module."""

    @pytest.fixture
    def module(self):
        """Return a Whoisfreaks module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
