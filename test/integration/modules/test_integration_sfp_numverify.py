# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_numverify.py
import pytest
...
# This test requires credentials for the Numverify service
# To run this test, set the environment variables:
# - SF_SFP_NUMVERIFY_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_NUMVERIFY_API_KEY']),
    reason="Integration test - requires Numverify credentials"
)
class TestModuleIntegrationNumverify:
    """Integration testing for the Numverify module."""

    @pytest.fixture
    def module(self):
        """Return a Numverify module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
