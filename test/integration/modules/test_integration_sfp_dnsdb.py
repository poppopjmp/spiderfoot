# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnsdb.py
import pytest
...
# This test requires credentials for the Dnsdb service
# To run this test, set the environment variables:
# - SF_SFP_DNSDB_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSDB_API_KEY']),
    reason="Integration test - requires Dnsdb credentials"
)
class TestModuleIntegrationDnsdb:
    """Integration testing for the Dnsdb module."""

    @pytest.fixture
    def module(self):
        """Return a Dnsdb module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
