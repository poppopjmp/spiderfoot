# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_whoxy.py
import pytest
...
# This test requires credentials for the Whoxy service
# To run this test, set the environment variables:
# - SF_SFP_WHOXY_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_WHOXY_API_KEY']),
    reason="Integration test - requires Whoxy credentials"
)
class TestModuleIntegrationWhoxy:
    """Integration testing for the Whoxy module."""

    @pytest.fixture
    def module(self):
        """Return a Whoxy module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
