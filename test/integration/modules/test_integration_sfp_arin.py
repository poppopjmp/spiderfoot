# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_arin.py
import pytest
...
# This test requires credentials for the Arin service
# To run this test, set the environment variables:
# - SF_SFP_ARIN_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ARIN_API_KEY']),
    reason="Integration test - requires Arin credentials"
)
class TestModuleIntegrationArin:
    """Integration testing for the Arin module."""

    @pytest.fixture
    def module(self):
        """Return a Arin module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
