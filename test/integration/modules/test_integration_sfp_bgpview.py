# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_bgpview.py
import pytest
import os
...
# This test requires credentials for the Bgpview service
# To run this test, set the environment variables:
# - SF_SFP_BGPVIEW_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_BGPVIEW_API_KEY"]),
    reason="Integration test - requires Bgpview credentials",
)
class TestModuleIntegrationBgpview:
    """Integration testing for the Bgpview module."""

    @pytest.fixture
    def module(self):
        """Return a Bgpview module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
