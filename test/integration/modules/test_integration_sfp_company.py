# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_company.py
import pytest
import os
...
# This test requires credentials for the Company service
# To run this test, set the environment variables:
# - SF_SFP_COMPANY_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_COMPANY_API_KEY"]),
    reason="Integration test - requires Company credentials",
)
class TestModuleIntegrationCompany:
    """Integration testing for the Company module."""

    @pytest.fixture
    def module(self):
        """Return a Company module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
