# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_binaryedge.py
import pytest
...
# This test requires credentials for the Binaryedge service
# To run this test, set the environment variables:
# - SF_SFP_BINARYEDGE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BINARYEDGE_API_KEY']),
    reason="Integration test - requires Binaryedge credentials"
)
class TestModuleIntegrationBinaryedge:
    """Integration testing for the Binaryedge module."""

    @pytest.fixture
    def module(self):
        """Return a Binaryedge module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
