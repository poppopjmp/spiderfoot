# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_netlas.py
import pytest
import os
...
# This test requires credentials for the Netlas service
# To run this test, set the environment variables:
# - SF_SFP_NETLAS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_NETLAS_API_KEY"]),
    reason="Integration test - requires Netlas credentials",
)
class TestModuleIntegrationNetlas:
    """Integration testing for the Netlas module."""

    @pytest.fixture
    def module(self):
        """Return a Netlas module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
