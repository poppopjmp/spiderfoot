# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_pulsedive.py
import pytest
...
# This test requires credentials for the Pulsedive service
# To run this test, set the environment variables:
# - SF_SFP_PULSEDIVE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_PULSEDIVE_API_KEY']),
    reason="Integration test - requires Pulsedive credentials"
)
class TestModuleIntegrationPulsedive:
    """Integration testing for the Pulsedive module."""

    @pytest.fixture
    def module(self):
        """Return a Pulsedive module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
