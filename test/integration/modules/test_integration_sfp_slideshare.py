# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_slideshare.py
import pytest
...
# This test requires credentials for the Slideshare service
# To run this test, set the environment variables:
# - SF_SFP_SLIDESHARE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SLIDESHARE_API_KEY']),
    reason="Integration test - requires Slideshare credentials"
)
class TestModuleIntegrationSlideshare:
    """Integration testing for the Slideshare module."""

    @pytest.fixture
    def module(self):
        """Return a Slideshare module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
