# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_flickr.py
import pytest
...
# This test requires credentials for the Flickr service
# To run this test, set the environment variables:
# - SF_SFP_FLICKR_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FLICKR_API_KEY']),
    reason="Integration test - requires Flickr credentials"
)
class TestModuleIntegrationFlickr:
    """Integration testing for the Flickr module."""

    @pytest.fixture
    def module(self):
        """Return a Flickr module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
