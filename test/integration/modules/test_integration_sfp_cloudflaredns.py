# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_cloudflaredns.py
import pytest
...
# This test requires credentials for the Cloudflaredns service
# To run this test, set the environment variables:
# - SF_SFP_CLOUDFLAREDNS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CLOUDFLAREDNS_API_KEY']),
    reason="Integration test - requires Cloudflaredns credentials"
)
class TestModuleIntegrationCloudflaredns:
    """Integration testing for the Cloudflaredns module."""

    @pytest.fixture
    def module(self):
        """Return a Cloudflaredns module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
