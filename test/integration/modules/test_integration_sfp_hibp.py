# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_hibp.py
import pytest
...
# This test requires credentials for the H Ibp service
# To run this test, set the environment variables:
# - SF_SFP_HIBP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_HIBP_API_KEY']),
    reason="Integration test - requires H Ibp credentials"
)
class TestModuleIntegrationhIbp:
    """Integration testing for the H Ibp module."""

    @pytest.fixture
    def module(self):
        """Return a H Ibp module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
