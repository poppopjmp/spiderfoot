# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_errors.py
import pytest
...
# This test requires credentials for the Errors service
# To run this test, set the environment variables:
# - SF_SFP_ERRORS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ERRORS_API_KEY']),
    reason="Integration test - requires Errors credentials"
)
class TestModuleIntegrationErrors:
    """Integration testing for the Errors module."""

    @pytest.fixture
    def module(self):
        """Return a Errors module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
