# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_apple_itunes.py
import pytest
...
# This test requires credentials for the Apple Itunes service
# To run this test, set the environment variables:
# - SF_SFP_APPLE_ITUNES_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_APPLE_ITUNES_API_KEY']),
    reason="Integration test - requires Apple Itunes credentials"
)
class TestModuleIntegrationAppleItunes:
    """Integration testing for the Apple Itunes module."""

    @pytest.fixture
    def module(self):
        """Return a Apple Itunes module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
