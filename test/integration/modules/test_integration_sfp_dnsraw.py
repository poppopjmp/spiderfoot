# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnsraw.py
import pytest
...
# This test requires credentials for the Dnsraw service
# To run this test, set the environment variables:
# - SF_SFP_DNSRAW_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSRAW_API_KEY']),
    reason="Integration test - requires Dnsraw credentials"
)
class TestModuleIntegrationDnsraw:
    """Integration testing for the Dnsraw module."""

    @pytest.fixture
    def module(self):
        """Return a Dnsraw module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
