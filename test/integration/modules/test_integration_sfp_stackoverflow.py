# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_stackoverflow.py
import pytest
...
# This test requires credentials for the Stackoverflow service
# To run this test, set the environment variables:
# - SF_SFP_STACKOVERFLOW_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_STACKOVERFLOW_API_KEY']),
    reason="Integration test - requires Stackoverflow credentials"
)
class TestModuleIntegrationStackoverflow:
    """Integration testing for the Stackoverflow module."""

    @pytest.fixture
    def module(self):
        """Return a Stackoverflow module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
