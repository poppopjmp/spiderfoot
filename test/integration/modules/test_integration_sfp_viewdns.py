# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_viewdns.py
import pytest
import os
...
# This test requires credentials for the Viewdns service
# To run this test, set the environment variables:
# - SF_SFP_VIEWDNS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_VIEWDNS_API_KEY"]),
    reason="Integration test - requires Viewdns credentials",
)
class TestModuleIntegrationViewdns:
    """Integration testing for the Viewdns module."""

    @pytest.fixture
    def module(self):
        """Return a Viewdns module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
