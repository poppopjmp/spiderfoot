# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_fortinet.py
import pytest
...
# This test requires credentials for the Fortinet service
# To run this test, set the environment variables:
# - SF_SFP_FORTINET_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FORTINET_API_KEY']),
    reason="Integration test - requires Fortinet credentials"
)
class TestModuleIntegrationFortinet:
    """Integration testing for the Fortinet module."""

    @pytest.fixture
    def module(self):
        """Return a Fortinet module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
