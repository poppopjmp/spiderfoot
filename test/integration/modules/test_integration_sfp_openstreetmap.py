# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_openstreetmap.py
import pytest
...
# This test requires credentials for the Openstreetmap service
# To run this test, set the environment variables:
# - SF_SFP_OPENSTREETMAP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_OPENSTREETMAP_API_KEY']),
    reason="Integration test - requires Openstreetmap credentials"
)
class TestModuleIntegrationOpenstreetmap:
    """Integration testing for the Openstreetmap module."""

    @pytest.fixture
    def module(self):
        """Return a Openstreetmap module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
