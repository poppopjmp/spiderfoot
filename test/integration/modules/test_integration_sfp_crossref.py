# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_crossref.py
import pytest
...
# This test requires credentials for the Crossref service
# To run this test, set the environment variables:
# - SF_SFP_CROSSREF_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CROSSREF_API_KEY']),
    reason="Integration test - requires Crossref credentials"
)
class TestModuleIntegrationCrossref:
    """Integration testing for the Crossref module."""

    @pytest.fixture
    def module(self):
        """Return a Crossref module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
