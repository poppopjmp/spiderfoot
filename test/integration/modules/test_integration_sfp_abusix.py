# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_abusix.py
import pytest
import os
...
# This test requires credentials for the Abusix service
# To run this test, set the environment variables:
# - SF_SFP_ABUSIX_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_ABUSIX_API_KEY"]),
    reason="Integration test - requires Abusix credentials",
)
class TestModuleIntegrationAbusix:
    """Integration testing for the Abusix module."""

    @pytest.fixture
    def module(self):
        """Return a Abusix module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
