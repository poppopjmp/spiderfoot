# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_bitcoinwhoswho.py
import pytest
...
# This test requires credentials for the Bitcoinwhoswho service
# To run this test, set the environment variables:
# - SF_SFP_BITCOINWHOSWHO_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BITCOINWHOSWHO_API_KEY']),
    reason="Integration test - requires Bitcoinwhoswho credentials"
)
class TestModuleIntegrationBitcoinwhoswho:
    """Integration testing for the Bitcoinwhoswho module."""

    @pytest.fixture
    def module(self):
        """Return a Bitcoinwhoswho module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
