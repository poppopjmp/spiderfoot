# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_robtex.py
import pytest
...
# This test requires credentials for the Robtex service
# To run this test, set the environment variables:
# - SF_SFP_ROBTEX_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ROBTEX_API_KEY']),
    reason="Integration test - requires Robtex credentials"
)
class TestModuleIntegrationRobtex:
    """Integration testing for the Robtex module."""

    @pytest.fixture
    def module(self):
        """Return a Robtex module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
