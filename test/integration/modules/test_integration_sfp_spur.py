# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_spur.py
import pytest
...
# This test requires credentials for the Spur service
# To run this test, set the environment variables:
# - SF_SFP_SPUR_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SPUR_API_KEY']),
    reason="Integration test - requires Spur credentials"
)
class TestModuleIntegrationSpur:
    """Integration testing for the Spur module."""

    @pytest.fixture
    def module(self):
        """Return a Spur module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
