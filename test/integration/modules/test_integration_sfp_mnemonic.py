# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_mnemonic.py
import pytest
...
# This test requires credentials for the Mnemonic service
# To run this test, set the environment variables:
# - SF_SFP_MNEMONIC_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_MNEMONIC_API_KEY']),
    reason="Integration test - requires Mnemonic credentials"
)
class TestModuleIntegrationMnemonic:
    """Integration testing for the Mnemonic module."""

    @pytest.fixture
    def module(self):
        """Return a Mnemonic module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
