# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_coinblocker.py
import pytest
...
# This test requires credentials for the Coinblocker service
# To run this test, set the environment variables:
# - SF_SFP_COINBLOCKER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_COINBLOCKER_API_KEY']),
    reason="Integration test - requires Coinblocker credentials"
)
class TestModuleIntegrationCoinblocker:
    """Integration testing for the Coinblocker module."""

    @pytest.fixture
    def module(self):
        """Return a Coinblocker module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
