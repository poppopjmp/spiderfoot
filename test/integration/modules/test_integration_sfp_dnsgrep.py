# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnsgrep.py
import pytest
...
# This test requires credentials for the Dnsgrep service
# To run this test, set the environment variables:
# - SF_SFP_DNSGREP_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSGREP_API_KEY']),
    reason="Integration test - requires Dnsgrep credentials"
)
class TestModuleIntegrationDnsgrep:
    """Integration testing for the Dnsgrep module."""

    @pytest.fixture
    def module(self):
        """Return a Dnsgrep module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
