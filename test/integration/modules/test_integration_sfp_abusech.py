# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_abusech.py
import pytest
import os
...
# This test requires credentials for the Abusech service
# To run this test, set the environment variables:
# - SF_SFP_ABUSECH_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_ABUSECH_API_KEY"]),
    reason="Integration test - requires Abusech credentials",
)
class TestModuleIntegrationAbusech:
    """Integration testing for the Abusech module."""

    @pytest.fixture
    def module(self):
        """Return a Abusech module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
