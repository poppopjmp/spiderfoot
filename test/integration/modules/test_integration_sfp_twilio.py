# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_twilio.py
import pytest
import os
...
# This test requires credentials for the Twilio service
# To run this test, set the environment variables:
# - SF_SFP_TWILIO_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_TWILIO_API_KEY"]),
    reason="Integration test - requires Twilio credentials",
)
class TestModuleIntegrationTwilio:
    """Integration testing for the Twilio module."""

    @pytest.fixture
    def module(self):
        """Return a Twilio module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
