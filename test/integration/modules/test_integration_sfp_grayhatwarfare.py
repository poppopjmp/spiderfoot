# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_grayhatwarfare.py
import pytest
...
# This test requires credentials for the Grayhatwarfare service
# To run this test, set the environment variables:
# - SF_SFP_GRAYHATWARFARE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_GRAYHATWARFARE_API_KEY']),
    reason="Integration test - requires Grayhatwarfare credentials"
)
class TestModuleIntegrationGrayhatwarfare:
    """Integration testing for the Grayhatwarfare module."""

    @pytest.fixture
    def module(self):
        """Return a Grayhatwarfare module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
