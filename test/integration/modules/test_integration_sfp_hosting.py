# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_hosting.py
import pytest
...
# This test requires credentials for the H Osting service
# To run this test, set the environment variables:
# - SF_SFP_HOSTING_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_HOSTING_API_KEY']),
    reason="Integration test - requires H Osting credentials"
)
class TestModuleIntegrationhOsting:
    """Integration testing for the H Osting module."""

    @pytest.fixture
    def module(self):
        """Return a H Osting module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
