# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_riskiq.py
import pytest
import os
...
# This test requires credentials for the Riskiq service
# To run this test, set the environment variables:
# - SF_SFP_RISKIQ_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_RISKIQ_API_KEY"]),
    reason="Integration test - requires Riskiq credentials",
)
class TestModuleIntegrationRiskiq:
    """Integration testing for the Riskiq module."""

    @pytest.fixture
    def module(self):
        """Return a Riskiq module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
