# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_opennic.py
import pytest
import os
...
# This test requires credentials for the Opennic service
# To run this test, set the environment variables:
# - SF_SFP_OPENNIC_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_OPENNIC_API_KEY"]),
    reason="Integration test - requires Opennic credentials",
)
class TestModuleIntegrationOpennic:
    """Integration testing for the Opennic module."""

    @pytest.fixture
    def module(self):
        """Return a Opennic module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
