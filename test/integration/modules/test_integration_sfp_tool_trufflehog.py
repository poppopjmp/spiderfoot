# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_tool_trufflehog.py
import pytest
...
# This test requires credentials for the Tool Trufflehog service
# To run this test, set the environment variables:
# - SF_SFP_TOOL_TRUFFLEHOG_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TOOL_TRUFFLEHOG_API_KEY']),
    reason="Integration test - requires Tool Trufflehog credentials"
)
class TestModuleIntegrationToolTrufflehog:
    """Integration testing for the Tool Trufflehog module."""

    @pytest.fixture
    def module(self):
        """Return a Tool Trufflehog module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
