# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_threatfox.py
import pytest
...
# This test requires credentials for the Threatfox service
# To run this test, set the environment variables:
# - SF_SFP_THREATFOX_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_THREATFOX_API_KEY']),
    reason="Integration test - requires Threatfox credentials"
)
class TestModuleIntegrationThreatfox:
    """Integration testing for the Threatfox module."""

    @pytest.fixture
    def module(self):
        """Return a Threatfox module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
