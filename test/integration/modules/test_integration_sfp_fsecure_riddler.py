# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_fsecure_riddler.py
import pytest
...
# This test requires credentials for the Fsecure Riddler service
# To run this test, set the environment variables:
# - SF_SFP_FSECURE_RIDDLER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FSECURE_RIDDLER_API_KEY']),
    reason="Integration test - requires Fsecure Riddler credentials"
)
class TestModuleIntegrationFsecureRiddler:
    """Integration testing for the Fsecure Riddler module."""

    @pytest.fixture
    def module(self):
        """Return a Fsecure Riddler module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
