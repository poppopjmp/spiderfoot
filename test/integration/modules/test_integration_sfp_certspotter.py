# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_certspotter.py
import pytest
...
# This test requires credentials for the Certspotter service
# To run this test, set the environment variables:
# - SF_SFP_CERTSPOTTER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CERTSPOTTER_API_KEY']),
    reason="Integration test - requires Certspotter credentials"
)
class TestModuleIntegrationCertspotter:
    """Integration testing for the Certspotter module."""

    @pytest.fixture
    def module(self):
        """Return a Certspotter module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
