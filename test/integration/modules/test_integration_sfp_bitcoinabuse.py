# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_bitcoinabuse.py
import pytest
...
# This test requires credentials for the Bitcoinabuse service
# To run this test, set the environment variables:
# - SF_SFP_BITCOINABUSE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BITCOINABUSE_API_KEY']),
    reason="Integration test - requires Bitcoinabuse credentials"
)
class TestModuleIntegrationBitcoinabuse:
    """Integration testing for the Bitcoinabuse module."""

    @pytest.fixture
    def module(self):
        """Return a Bitcoinabuse module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
