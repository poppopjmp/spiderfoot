# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_securitytrails.py
import pytest
...
# This test requires credentials for the Securitytrails service
# To run this test, set the environment variables:
# - SF_SFP_SECURITYTRAILS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SECURITYTRAILS_API_KEY']),
    reason="Integration test - requires Securitytrails credentials"
)
class TestModuleIntegrationSecuritytrails:
    """Integration testing for the Securitytrails module."""

    @pytest.fixture
    def module(self):
        """Return a Securitytrails module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
