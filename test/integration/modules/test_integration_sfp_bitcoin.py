# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_bitcoin.py
import pytest
...
# This test requires credentials for the Bitcoin service
# To run this test, set the environment variables:
# - SF_SFP_BITCOIN_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BITCOIN_API_KEY']),
    reason="Integration test - requires Bitcoin credentials"
)
class TestModuleIntegrationBitcoin:
    """Integration testing for the Bitcoin module."""

    @pytest.fixture
    def module(self):
        """Return a Bitcoin module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
