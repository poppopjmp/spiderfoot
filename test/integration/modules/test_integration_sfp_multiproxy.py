# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_multiproxy.py
import pytest
...
# This test requires credentials for the Multiproxy service
# To run this test, set the environment variables:
# - SF_SFP_MULTIPROXY_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_MULTIPROXY_API_KEY']),
    reason="Integration test - requires Multiproxy credentials"
)
class TestModuleIntegrationMultiproxy:
    """Integration testing for the Multiproxy module."""

    @pytest.fixture
    def module(self):
        """Return a Multiproxy module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
