# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_azureblobstorage.py
import pytest
...
# This test requires credentials for the Azureblobstorage service
# To run this test, set the environment variables:
# - SF_SFP_AZUREBLOBSTORAGE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_AZUREBLOBSTORAGE_API_KEY']),
    reason="Integration test - requires Azureblobstorage credentials"
)
class TestModuleIntegrationAzureblobstorage:
    """Integration testing for the Azureblobstorage module."""

    @pytest.fixture
    def module(self):
        """Return a Azureblobstorage module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
