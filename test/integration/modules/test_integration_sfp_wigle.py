# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_wigle.py
import pytest
import os
...
# This test requires credentials for the Wigle service
# To run this test, set the environment variables:
# - SF_SFP_WIGLE_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_WIGLE_API_KEY"]),
    reason="Integration test - requires Wigle credentials",
)
class TestModuleIntegrationWigle:
    """Integration testing for the Wigle module."""

    @pytest.fixture
    def module(self):
        """Return a Wigle module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
