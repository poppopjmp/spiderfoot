# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_customfeed.py
import pytest
...
# This test requires credentials for the Customfeed service
# To run this test, set the environment variables:
# - SF_SFP_CUSTOMFEED_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_CUSTOMFEED_API_KEY']),
    reason="Integration test - requires Customfeed credentials"
)
class TestModuleIntegrationCustomfeed:
    """Integration testing for the Customfeed module."""

    @pytest.fixture
    def module(self):
        """Return a Customfeed module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
