# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_googlesearch.py
import pytest
...
# This test requires credentials for the Googlesearch service
# To run this test, set the environment variables:
# - SF_SFP_GOOGLESEARCH_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_GOOGLESEARCH_API_KEY']),
    reason="Integration test - requires Googlesearch credentials"
)
class TestModuleIntegrationGooglesearch:
    """Integration testing for the Googlesearch module."""

    @pytest.fixture
    def module(self):
        """Return a Googlesearch module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
