# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_countryname.py
import pytest
...
# This test requires credentials for the Countryname service
# To run this test, set the environment variables:
# - SF_SFP_COUNTRYNAME_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_COUNTRYNAME_API_KEY']),
    reason="Integration test - requires Countryname credentials"
)
class TestModuleIntegrationCountryname:
    """Integration testing for the Countryname module."""

    @pytest.fixture
    def module(self):
        """Return a Countryname module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
