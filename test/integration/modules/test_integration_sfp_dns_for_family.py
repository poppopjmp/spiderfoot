# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dns_for_family.py
import pytest
...
# This test requires credentials for the Dns For Family service
# To run this test, set the environment variables:
# - SF_SFP_DNS_FOR_FAMILY_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNS_FOR_FAMILY_API_KEY']),
    reason="Integration test - requires Dns For Family credentials"
)
class TestModuleIntegrationDnsForFamily:
    """Integration testing for the Dns For Family module."""

    @pytest.fixture
    def module(self):
        """Return a Dns For Family module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
