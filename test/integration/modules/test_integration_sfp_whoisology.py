# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_whoisology.py
import pytest
...
# This test requires credentials for the Whoisology service
# To run this test, set the environment variables:
# - SF_SFP_WHOISOLOGY_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_WHOISOLOGY_API_KEY']),
    reason="Integration test - requires Whoisology credentials"
)
class TestModuleIntegrationWhoisology:
    """Integration testing for the Whoisology module."""

    @pytest.fixture
    def module(self):
        """Return a Whoisology module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
