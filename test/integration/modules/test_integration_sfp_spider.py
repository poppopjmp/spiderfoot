# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_spider.py
import pytest
...
# This test requires credentials for the Spider service
# To run this test, set the environment variables:
# - SF_SFP_SPIDER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SPIDER_API_KEY']),
    reason="Integration test - requires Spider credentials"
)
class TestModuleIntegrationSpider:
    """Integration testing for the Spider module."""

    @pytest.fixture
    def module(self):
        """Return a Spider module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
