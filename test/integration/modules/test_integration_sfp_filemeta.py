# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_filemeta.py
import pytest
...
# This test requires credentials for the Filemeta service
# To run this test, set the environment variables:
# - SF_SFP_FILEMETA_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FILEMETA_API_KEY']),
    reason="Integration test - requires Filemeta credentials"
)
class TestModuleIntegrationFilemeta:
    """Integration testing for the Filemeta module."""

    @pytest.fixture
    def module(self):
        """Return a Filemeta module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
