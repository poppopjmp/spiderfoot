# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_threatjammer.py
import pytest
...
# This test requires credentials for the Threatjammer service
# To run this test, set the environment variables:
# - SF_SFP_THREATJAMMER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_THREATJAMMER_API_KEY']),
    reason="Integration test - requires Threatjammer credentials"
)
class TestModuleIntegrationThreatjammer:
    """Integration testing for the Threatjammer module."""

    @pytest.fixture
    def module(self):
        """Return a Threatjammer module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
