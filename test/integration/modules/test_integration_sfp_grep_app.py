# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_grep_app.py
import pytest
...
# This test requires credentials for the Grep App service
# To run this test, set the environment variables:
# - SF_SFP_GREP_APP_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_GREP_APP_API_KEY']),
    reason="Integration test - requires Grep App credentials"
)
class TestModuleIntegrationGrepApp:
    """Integration testing for the Grep App module."""

    @pytest.fixture
    def module(self):
        """Return a Grep App module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
