# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_cookie.py
import pytest
...
# This test requires credentials for the Cookie service
# To run this test, set the environment variables:
# - SF_SFP_COOKIE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_COOKIE_API_KEY']),
    reason="Integration test - requires Cookie credentials"
)
class TestModuleIntegrationCookie:
    """Integration testing for the Cookie module."""

    @pytest.fixture
    def module(self):
        """Return a Cookie module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
