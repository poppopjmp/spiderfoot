# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_etherscan.py
import pytest
...
# This test requires credentials for the Etherscan service
# To run this test, set the environment variables:
# - SF_SFP_ETHERSCAN_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ETHERSCAN_API_KEY']),
    reason="Integration test - requires Etherscan credentials"
)
class TestModuleIntegrationEtherscan:
    """Integration testing for the Etherscan module."""

    @pytest.fixture
    def module(self):
        """Return a Etherscan module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
