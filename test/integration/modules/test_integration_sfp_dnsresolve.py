# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnsresolve.py
import pytest
...
# This test requires credentials for the Dnsresolve service
# To run this test, set the environment variables:
# - SF_SFP_DNSRESOLVE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSRESOLVE_API_KEY']),
    reason="Integration test - requires Dnsresolve credentials"
)
class TestModuleIntegrationDnsresolve:
    """Integration testing for the Dnsresolve module."""

    @pytest.fixture
    def module(self):
        """Return a Dnsresolve module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
