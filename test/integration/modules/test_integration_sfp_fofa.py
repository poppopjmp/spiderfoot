# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_fofa.py
import pytest
...
# This test requires credentials for the Fofa service
# To run this test, set the environment variables:
# - SF_SFP_FOFA_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FOFA_API_KEY']),
    reason="Integration test - requires Fofa credentials"
)
class TestModuleIntegrationFofa:
    """Integration testing for the Fofa module."""

    @pytest.fixture
    def module(self):
        """Return a Fofa module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
