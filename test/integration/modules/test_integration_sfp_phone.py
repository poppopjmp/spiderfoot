# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_phone.py
import pytest
...
# This test requires credentials for the Phone service
# To run this test, set the environment variables:
# - SF_SFP_PHONE_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_PHONE_API_KEY']),
    reason="Integration test - requires Phone credentials"
)
class TestModuleIntegrationPhone:
    """Integration testing for the Phone module."""

    @pytest.fixture
    def module(self):
        """Return a Phone module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
