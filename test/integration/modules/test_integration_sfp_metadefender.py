# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_metadefender.py
import pytest
...
# This test requires credentials for the Metadefender service
# To run this test, set the environment variables:
# - SF_SFP_METADEFENDER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_METADEFENDER_API_KEY']),
    reason="Integration test - requires Metadefender credentials"
)
class TestModuleIntegrationMetadefender:
    """Integration testing for the Metadefender module."""

    @pytest.fixture
    def module(self):
        """Return a Metadefender module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
