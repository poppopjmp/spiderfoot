# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_fraudguard.py
import pytest
...
# This test requires credentials for the Fraudguard service
# To run this test, set the environment variables:
# - SF_SFP_FRAUDGUARD_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_FRAUDGUARD_API_KEY']),
    reason="Integration test - requires Fraudguard credentials"
)
class TestModuleIntegrationFraudguard:
    """Integration testing for the Fraudguard module."""

    @pytest.fixture
    def module(self):
        """Return a Fraudguard module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
