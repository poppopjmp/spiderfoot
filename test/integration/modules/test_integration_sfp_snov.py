# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_snov.py
import pytest
...
# This test requires credentials for the Snov service
# To run this test, set the environment variables:
# - SF_SFP_SNOV_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SNOV_API_KEY']),
    reason="Integration test - requires Snov credentials"
)
class TestModuleIntegrationSnov:
    """Integration testing for the Snov module."""

    @pytest.fixture
    def module(self):
        """Return a Snov module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
