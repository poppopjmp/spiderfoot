# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_networksdb.py
import pytest
...
# This test requires credentials for the Networksdb service
# To run this test, set the environment variables:
# - SF_SFP_NETWORKSDB_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_NETWORKSDB_API_KEY']),
    reason="Integration test - requires Networksdb credentials"
)
class TestModuleIntegrationNetworksdb:
    """Integration testing for the Networksdb module."""

    @pytest.fixture
    def module(self):
        """Return a Networksdb module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
