# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_hunter.py
import pytest
...
# This test requires credentials for the H Unter service
# To run this test, set the environment variables:
# - SF_SFP_HUNTER_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_HUNTER_API_KEY']),
    reason="Integration test - requires H Unter credentials"
)
class TestModuleIntegrationhUnter:
    """Integration testing for the H Unter module."""

    @pytest.fixture
    def module(self):
        """Return a H Unter module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
