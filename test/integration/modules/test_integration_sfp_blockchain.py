# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_blockchain.py
import pytest
...
# This test requires credentials for the Blockchain service
# To run this test, set the environment variables:
# - SF_SFP_BLOCKCHAIN_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BLOCKCHAIN_API_KEY']),
    reason="Integration test - requires Blockchain credentials"
)
class TestModuleIntegrationBlockchain:
    """Integration testing for the Blockchain module."""

    @pytest.fixture
    def module(self):
        """Return a Blockchain module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
