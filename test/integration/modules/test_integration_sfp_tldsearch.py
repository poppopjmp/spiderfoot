# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_tldsearch.py
import pytest
...
# This test requires credentials for the Tldsearch service
# To run this test, set the environment variables:
# - SF_SFP_TLDSEARCH_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TLDSEARCH_API_KEY']),
    reason="Integration test - requires Tldsearch credentials"
)
class TestModuleIntegrationTldsearch:
    """Integration testing for the Tldsearch module."""

    @pytest.fixture
    def module(self):
        """Return a Tldsearch module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
