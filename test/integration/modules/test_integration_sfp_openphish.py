# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_openphish.py
import pytest
...
# This test requires credentials for the Openphish service
# To run this test, set the environment variables:
# - SF_SFP_OPENPHISH_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_OPENPHISH_API_KEY']),
    reason="Integration test - requires Openphish credentials"
)
class TestModuleIntegrationOpenphish:
    """Integration testing for the Openphish module."""

    @pytest.fixture
    def module(self):
        """Return a Openphish module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
