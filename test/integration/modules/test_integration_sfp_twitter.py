# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_twitter.py
import pytest
...
# This test requires credentials for the Twitter service
# To run this test, set the environment variables:
# - SF_SFP_TWITTER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TWITTER_API_KEY']),
    reason="Integration test - requires Twitter credentials"
)
class TestModuleIntegrationTwitter:
    """Integration testing for the Twitter module."""

    @pytest.fixture
    def module(self):
        """Return a Twitter module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
