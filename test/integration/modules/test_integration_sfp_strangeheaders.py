# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_strangeheaders.py
import pytest
...
# This test requires credentials for the Strangeheaders service
# To run this test, set the environment variables:
# - SF_SFP_STRANGEHEADERS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_STRANGEHEADERS_API_KEY']),
    reason="Integration test - requires Strangeheaders credentials"
)
class TestModuleIntegrationStrangeheaders:
    """Integration testing for the Strangeheaders module."""

    @pytest.fixture
    def module(self):
        """Return a Strangeheaders module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
