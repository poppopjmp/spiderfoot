# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_trumail.py
import pytest
import os
...
# This test requires credentials for the Trumail service
# To run this test, set the environment variables:
# - SF_SFP_TRUMAIL_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_TRUMAIL_API_KEY"]),
    reason="Integration test - requires Trumail credentials",
)
class TestModuleIntegrationTrumail:
    """Integration testing for the Trumail module."""

    @pytest.fixture
    def module(self):
        """Return a Trumail module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
