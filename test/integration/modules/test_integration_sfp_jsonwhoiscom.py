# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_jsonwhoiscom.py
import pytest
...
# This test requires credentials for the Jsonwhoiscom service
# To run this test, set the environment variables:
# - SF_SFP_JSONWHOISCOM_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_JSONWHOISCOM_API_KEY']),
    reason="Integration test - requires Jsonwhoiscom credentials"
)
class TestModuleIntegrationJsonwhoiscom:
    """Integration testing for the Jsonwhoiscom module."""

    @pytest.fixture
    def module(self):
        """Return a Jsonwhoiscom module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
