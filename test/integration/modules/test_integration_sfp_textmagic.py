# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_textmagic.py
import pytest
...
# This test requires credentials for the Textmagic service
# To run this test, set the environment variables:
# - SF_SFP_TEXTMAGIC_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_TEXTMAGIC_API_KEY']),
    reason="Integration test - requires Textmagic credentials"
)
class TestModuleIntegrationTextmagic:
    """Integration testing for the Textmagic module."""

    @pytest.fixture
    def module(self):
        """Return a Textmagic module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
