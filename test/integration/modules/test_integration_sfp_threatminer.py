# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_threatminer.py
import pytest
...
# This test requires credentials for the Threatminer service
# To run this test, set the environment variables:
# - SF_SFP_THREATMINER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_THREATMINER_API_KEY']),
    reason="Integration test - requires Threatminer credentials"
)
class TestModuleIntegrationThreatminer:
    """Integration testing for the Threatminer module."""

    @pytest.fixture
    def module(self):
        """Return a Threatminer module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
