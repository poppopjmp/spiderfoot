# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_pageinfo.py
import pytest
...
# This test requires credentials for the Pageinfo service
# To run this test, set the environment variables:
# - SF_SFP_PAGEINFO_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_PAGEINFO_API_KEY']),
    reason="Integration test - requires Pageinfo credentials"
)
class TestModuleIntegrationPageinfo:
    """Integration testing for the Pageinfo module."""

    @pytest.fixture
    def module(self):
        """Return a Pageinfo module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
