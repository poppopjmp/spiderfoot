# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_botscout.py
import pytest
...
# This test requires credentials for the Botscout service
# To run this test, set the environment variables:
# - SF_SFP_BOTSCOUT_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BOTSCOUT_API_KEY']),
    reason="Integration test - requires Botscout credentials"
)
class TestModuleIntegrationBotscout:
    """Integration testing for the Botscout module."""

    @pytest.fixture
    def module(self):
        """Return a Botscout module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
