# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_sorbs.py
import pytest
import os
...
# This test requires credentials for the Sorbs service
# To run this test, set the environment variables:
# - SF_SFP_SORBS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_SORBS_API_KEY"]),
    reason="Integration test - requires Sorbs credentials",
)
class TestModuleIntegrationSorbs:
    """Integration testing for the Sorbs module."""

    @pytest.fixture
    def module(self):
        """Return a Sorbs module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
