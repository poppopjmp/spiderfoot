# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_google_tag_manager.py
import pytest
...
# This test requires credentials for the Google Tag Manager service
# To run this test, set the environment variables:
# - SF_SFP_GOOGLE_TAG_MANAGER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_GOOGLE_TAG_MANAGER_API_KEY']),
    reason="Integration test - requires Google Tag Manager credentials"
)
class TestModuleIntegrationGoogleTagManager:
    """Integration testing for the Google Tag Manager module."""

    @pytest.fixture
    def module(self):
        """Return a Google Tag Manager module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
