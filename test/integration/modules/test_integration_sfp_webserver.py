# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_webserver.py
import pytest
...
# This test requires credentials for the Webserver service
# To run this test, set the environment variables:
# - SF_SFP_WEBSERVER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_WEBSERVER_API_KEY']),
    reason="Integration test - requires Webserver credentials"
)
class TestModuleIntegrationWebserver:
    """Integration testing for the Webserver module."""

    @pytest.fixture
    def module(self):
        """Return a Webserver module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
