# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_ripe.py
import pytest
...
# This test requires credentials for the Ripe service
# To run this test, set the environment variables:
# - SF_SFP_RIPE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_RIPE_API_KEY']),
    reason="Integration test - requires Ripe credentials"
)
class TestModuleIntegrationRipe:
    """Integration testing for the Ripe module."""

    @pytest.fixture
    def module(self):
        """Return a Ripe module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
