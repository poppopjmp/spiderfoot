# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_abuseipdb.py
import pytest
...
# This test requires credentials for the Abuseipdb service
# To run this test, set the environment variables:
# - SF_SFP_ABUSEIPDB_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_ABUSEIPDB_API_KEY']),
    reason="Integration test - requires Abuseipdb credentials"
)
class TestModuleIntegrationAbuseipdb:
    """Integration testing for the Abuseipdb module."""

    @pytest.fixture
    def module(self):
        """Return a Abuseipdb module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
