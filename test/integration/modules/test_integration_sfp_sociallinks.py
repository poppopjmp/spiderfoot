# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_sociallinks.py
import pytest
...
# This test requires credentials for the Sociallinks service
# To run this test, set the environment variables:
# - SF_SFP_SOCIALLINKS_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SOCIALLINKS_API_KEY']),
    reason="Integration test - requires Sociallinks credentials"
)
class TestModuleIntegrationSociallinks:
    """Integration testing for the Sociallinks module."""

    @pytest.fixture
    def module(self):
        """Return a Sociallinks module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
