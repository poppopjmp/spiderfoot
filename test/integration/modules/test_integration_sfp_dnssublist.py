# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_dnssublist.py
import pytest
...
# This test requires credentials for the Dnssublist service
# To run this test, set the environment variables:
# - SF_SFP_DNSSUBLIST_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_DNSSUBLIST_API_KEY']),
    reason="Integration test - requires Dnssublist credentials"
)
class TestModuleIntegrationDnssublist:
    """Integration testing for the Dnssublist module."""

    @pytest.fixture
    def module(self):
        """Return a Dnssublist module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
