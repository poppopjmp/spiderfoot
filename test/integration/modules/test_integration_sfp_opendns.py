# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_opendns.py
import pytest
...
# This test requires credentials for the Opendns service
# To run this test, set the environment variables:
# - SF_SFP_OPENDNS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_OPENDNS_API_KEY']),
    reason="Integration test - requires Opendns credentials"
)
class TestModuleIntegrationOpendns:
    """Integration testing for the Opendns module."""

    @pytest.fixture
    def module(self):
        """Return a Opendns module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
