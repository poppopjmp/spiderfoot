# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_names.py
import pytest
...
# This test requires credentials for the Names service
# To run this test, set the environment variables:
# - SF_SFP_NAMES_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_NAMES_API_KEY']),
    reason="Integration test - requires Names credentials"
)
class TestModuleIntegrationNames:
    """Integration testing for the Names module."""

    @pytest.fixture
    def module(self):
        """Return a Names module."""
        sf = SpiderFoot({
            '_debug': True,
            '__logging': True,
            '__outputfilter': None,
