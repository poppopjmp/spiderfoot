# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_whois.py
import pytest
import os
...
# This test requires credentials for the Whois service
# To run this test, set the environment variables:
# - SF_SFP_WHOIS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_WHOIS_API_KEY"]),
    reason="Integration test - requires Whois credentials",
)
class TestModuleIntegrationWhois:
    """Integration testing for the Whois module."""

    @pytest.fixture
    def module(self):
        """Return a Whois module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
