# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_whatcms.py
import pytest
import os
...
# This test requires credentials for the Whatcms service
# To run this test, set the environment variables:
# - SF_SFP_WHATCMS_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_WHATCMS_API_KEY"]),
    reason="Integration test - requires Whatcms credentials",
)
class TestModuleIntegrationWhatcms:
    """Integration testing for the Whatcms module."""

    @pytest.fixture
    def module(self):
        """Return a Whatcms module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
