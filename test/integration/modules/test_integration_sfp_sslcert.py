# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_sslcert.py
import pytest
import os
...
# This test requires credentials for the Sslcert service
# To run this test, set the environment variables:
# - SF_SFP_SSLCERT_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_SSLCERT_API_KEY"]),
    reason="Integration test - requires Sslcert credentials",
)
class TestModuleIntegrationSslcert:
    """Integration testing for the Sslcert module."""

    @pytest.fixture
    def module(self):
        """Return a Sslcert module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
