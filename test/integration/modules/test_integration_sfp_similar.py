# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_similar.py
import pytest
import os
...
# This test requires credentials for the Similar service
# To run this test, set the environment variables:
# - SF_SFP_SIMILAR_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_SIMILAR_API_KEY"]),
    reason="Integration test - requires Similar credentials",
)
class TestModuleIntegrationSimilar:
    """Integration testing for the Similar module."""

    @pytest.fixture
    def module(self):
        """Return a Similar module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
