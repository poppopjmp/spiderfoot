# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_blocklistde.py
import pytest
...
# This test requires credentials for the Blocklistde service
# To run this test, set the environment variables:
# - SF_SFP_BLOCKLISTDE_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_BLOCKLISTDE_API_KEY']),
    reason="Integration test - requires Blocklistde credentials"
)
class TestModuleIntegrationBlocklistde:
    """Integration testing for the Blocklistde module."""

    @pytest.fixture
    def module(self):
        """Return a Blocklistde module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
