# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_emailformat.py
import pytest
...
# This test requires credentials for the Emailformat service
# To run this test, set the environment variables:
# - SF_SFP_EMAILFORMAT_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_EMAILFORMAT_API_KEY']),
    reason="Integration test - requires Emailformat credentials"
)
class TestModuleIntegrationEmailformat:
    """Integration testing for the Emailformat module."""

    @pytest.fixture
    def module(self):
        """Return a Emailformat module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
