# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_punkspider.py
import pytest
...
# This test requires credentials for the Punkspider service
# To run this test, set the environment variables:
# - SF_SFP_PUNKSPIDER_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_PUNKSPIDER_API_KEY']),
    reason="Integration test - requires Punkspider credentials"
)
class TestModuleIntegrationPunkspider:
    """Integration testing for the Punkspider module."""

    @pytest.fixture
    def module(self):
        """Return a Punkspider module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
