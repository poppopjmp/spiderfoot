# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_socialprofiles.py
import pytest
...
# This test requires credentials for the Socialprofiles service
# To run this test, set the environment variables:
# - SF_SFP_SOCIALPROFILES_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SOCIALPROFILES_API_KEY']),
    reason="Integration test - requires Socialprofiles credentials"
)
class TestModuleIntegrationSocialprofiles:
    """Integration testing for the Socialprofiles module."""

    @pytest.fixture
    def module(self):
        """Return a Socialprofiles module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
