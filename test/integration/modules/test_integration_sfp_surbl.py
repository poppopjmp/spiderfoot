# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_surbl.py
import pytest
...
# This test requires credentials for the Surbl service
# To run this test, set the environment variables:
# - SF_SFP_SURBL_API_KEY

@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ['SF_SFP_SURBL_API_KEY']),
    reason="Integration test - requires Surbl credentials"
)
class TestModuleIntegrationSurbl:
    """Integration testing for the Surbl module."""

    @pytest.fixture
    def module(self):
        """Return a Surbl module."""
        sf = SpiderFoot({
        '_debug': True,
        '__logging': True,
        '__outputfilter': None,
