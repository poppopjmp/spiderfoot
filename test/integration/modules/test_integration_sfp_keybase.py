# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_keybase.py
import pytest
import os
...
# This test requires credentials for the Keybase service
# To run this test, set the environment variables:
# - SF_SFP_KEYBASE_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_KEYBASE_API_KEY"]),
    reason="Integration test - requires Keybase credentials",
)
class TestModuleIntegrationKeybase:
    """Integration testing for the Keybase module."""

    @pytest.fixture
    def module(self):
        """Return a Keybase module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
