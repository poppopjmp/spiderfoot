# filepath: /mnt/c/Users/van1sh/Documents/GitHub/spiderfoot/test/integration/modules/test_integration_sfp_voipbl.py
import pytest
import os
...
# This test requires credentials for the Voipbl service
# To run this test, set the environment variables:
# - SF_SFP_VOIPBL_API_KEY


@pytest.mark.skipif(
    not all(os.environ.get(env_var) for env_var in ["SF_SFP_VOIPBL_API_KEY"]),
    reason="Integration test - requires Voipbl credentials",
)
class TestModuleIntegrationVoipbl:
    """Integration testing for the Voipbl module."""

    @pytest.fixture
    def module(self):
        """Return a Voipbl module."""
        sf = SpiderFoot(
            {
                "_debug": True,
                "__logging": True,
